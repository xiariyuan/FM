...
class NestedTensor(object):
    ...
    def __getitem__(self, item):
        ...
        return NestedTensor(tensors=self.tensors[item], mask=mask)
    ...
    def clone(self):
        ...
        return NestedTensor(tensors=self.tensors.clone(), mask=mask)
...
def nested_tensor_index_select(nested_tensor: NestedTensor, dim: int, index: torch.Tensor):
    ...
    selected_mask = None if mask is None else torch.index_select(input=mask, dim=dim, index=index).contiguous()
