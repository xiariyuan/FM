# M23-59 Relation-pretrained Hierarchical Identity Segmentation and Flow Feasibility — Preregistration
...
`outputs/mot20_m23_20260718/m23_59_relation_pretrained_hierarchical_flow_v1/external_dataset_manifest.json`

Manifest SHA-256 at preregistration: `ac3839305a1cb8f67ba0c537f96dcd051454d834861c4859868273297652bf70`.
...
Each positive continuation is paired with one hard alternative from the same physical video, a different identity, within 30 frames of the positive destination, geometry distance ≤3, ranked by:

`0.70 * appearance cosine + 0.30 * exp(-geometry distance)`.

Tie-break: higher hardness, lower identity ID, earlier frame, lower deterministic segment index.
...
- node class-balanced focal/BCE component, weight 1.0;
- conditional multi-boundary focal component, weight 1.0;
- boundary-count consistency, weight 0.2;
- outgoing successor listwise loss, weight 0.5;
- incoming predecessor listwise loss, weight 0.5;
- paired replacement loss, weight 0.5;
- catastrophic-risk loss, weight 1.5;
- sequence group-DRO update, fixed eta 0.1;
- edit sparsity/source-anchor regularization, weight 0.02.

HOTA is never an action-level or boundary-level training label.
...
- deterministic PyTorch algorithms, no early stopping.

Checkpoint selection uses only MOT17 physical validation sequences. Fixed validation composite:

`0.30 * node impurity PR-AUC + 0.35 * conditional boundary PR-AUC + 0.25 * pairwise successor R@1 + 0.10 * (1 - catastrophic false-link rate)`.
...
- optimizer: AdamW;
- weight decay: 1e-4;
- the validation model, identity scaler declaration, node threshold, and boundary threshold are written before validation labels are opened;
...
An edge with predicted catastrophic risk above the policy limit is ineligible. No candidate K, gap quota, edit budget, source-anchor weight, or edge-utility scan is allowed.

Policies:

- P0: byte-exact M23-46 no-op;
- P1: fixed risk limit 0.02 and actual one-sided 95% Clopper-Pearson catastrophic-risk UCB ≤2%;
- P2: fixed risk limit 0.05 and actual one-sided 95% Clopper-Pearson catastrophic-risk UCB ≤5%.
...
A learned outer policy, if any, is fine-tuned for exactly 10 epochs on all three outer-training sequences. Node and boundary thresholds are the medians of the three previously frozen inner-training-only thresholds. No held sequence statistic is used.
...
- each held fold is evaluated exactly once;
- four trackers are stitched;
