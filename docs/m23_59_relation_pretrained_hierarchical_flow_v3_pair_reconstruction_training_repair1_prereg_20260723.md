# M23-64-R1 repair preregistration — frozen-pool pair reconstruction and from-scratch relation training
...
The original M23-64 run is immutable and closed as `FAIL_TRAINING_INCOMPLETE`. Its read-only traceback is frozen: `v2.validation_metrics` passed conditional-boundary labels containing `-1` unknown values into `summarize_binary`, and sklearn then raised `ValueError: Expected 2D array, got 1D array instead`. The repair changes no R62/R63 artifact and does not overwrite the original R64.

The sole repair is a validation adapter around the unchanged v2 metric definition: before binary summaries, retain only labels exactly in `{0,1}` and finite scores. Unknown labels are excluded, never mapped to negative. Model architecture, loss, optimizer, seeds, epochs, batches, sampling, gap buckets, composite checkpoint rule, and all forbidden-scope guards are unchanged.

Original M23-64 script SHA256: `a3bed845b1f01ea5484c255b4bc22fac8b2230cd1c6835a1f9c3ef95dec36087`.
Repair wrapper SHA256 (after freeze): `ed7242cae656f0b31e7cdbce8ac35a5cfb303a70992d657e39fc52e0c19d19fb`.
