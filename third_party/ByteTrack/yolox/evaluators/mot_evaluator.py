...
from yolox.tracker.byte_tracker_local_conflict import ByteTrackerLocalConflict
...
def write_tracker_diagnostics(filename, payload):
    with open(filename, "w") as f:
        json.dump(payload, f, indent=2)
    logger.info("save tracker diagnostics to {}".format(filename))
    ...
    def evaluate(
        ...
        tracker_cls = (
            ...
            if (
                ...
                or getattr(self.args, "use_posthost_hierarchical_edit", False)
                or str(getattr(self.args, "local_conflict_dump_dir", "") or "").strip()
            )
            else BYTETracker
        )
        diagnostics_dir = str(getattr(self.args, "local_conflict_diagnostics_dir", "") or "").strip()
        if diagnostics_dir:
            os.makedirs(diagnostics_dir, exist_ok=True)
        tracker = tracker_cls(self.args)
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            with torch.no_grad():
                ...
                if frame_id == 1:
                    if len(results) != 0:
                        ...
                        if diagnostics_dir and hasattr(tracker, "get_local_conflict_diagnostics"):
                            diagnostics_filename = os.path.join(
                                diagnostics_dir,
                                "{}.json".format(video_names[video_id - 1]),
                            )
                            write_tracker_diagnostics(
                                diagnostics_filename,
                                tracker.get_local_conflict_diagnostics(),
                            )
                        if hasattr(tracker, "close_local_conflict_dump"):
                            tracker.close_local_conflict_dump()
                        results = []
                    tracker = tracker_cls(self.args)
                if hasattr(tracker, "set_sequence_name"):
                    tracker.set_sequence_name(video_name)
                elif hasattr(tracker, "sequence_name"):
            ...
            if cur_iter == len(self.dataloader) - 1:
                ...
                if diagnostics_dir and hasattr(tracker, "get_local_conflict_diagnostics"):
                    diagnostics_filename = os.path.join(
                        diagnostics_dir,
                        "{}.json".format(video_names[video_id]),
                    )
                    write_tracker_diagnostics(
                        diagnostics_filename,
                        tracker.get_local_conflict_diagnostics(),
