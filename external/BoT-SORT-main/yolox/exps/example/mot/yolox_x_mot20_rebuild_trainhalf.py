# encoding: utf-8
"""MOT20 detector rebuild phase-1 config: train_half -> val_half."""
import os
import torch
...
class Exp(MyExp):
    def __init__(self):
        ...
        input_h = int(os.getenv("MOT20_DET_INPUT_H", "896"))
        input_w = int(os.getenv("MOT20_DET_INPUT_W", "1600"))
        test_h = int(os.getenv("MOT20_DET_TEST_H", str(input_h)))
        test_w = int(os.getenv("MOT20_DET_TEST_W", str(input_w)))
        self.input_size = (input_h, input_w)
        self.test_size = (test_h, test_w)
        self.random_size = (max(10, input_h // 32), max(10, input_w // 32))
        self.max_epoch = int(os.getenv("MOT20_DET_MAX_EPOCH", "20"))
        self.print_interval = 20
        self.eval_interval = max(1, int(os.getenv("MOT20_DET_EVAL_INTERVAL", "2")))
        self.no_aug_epochs = min(5, self.max_epoch)
        self.basic_lr_per_img = float(os.getenv("MOT20_DET_LR_PER_IMG", "0.0005")) / 64.0
        self.warmup_epochs = 1
        self.test_conf = 0.001
        self.nmsthre = 0.7
        self.data_num_workers = int(os.getenv("MOT20_DET_WORKERS", "4"))
    ...
    def get_data_loader(self, batch_size, is_distributed, no_aug=False):
        ...
        dataset = MOTDataset(
            ...
            preproc=TrainTransform(rgb_means=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225), max_labels=1200),
        )
        dataset = self._maybe_truncate(dataset, "MOT20_DET_SMOKE_TRAIN_IMAGES")
    ...
    def get_eval_loader(self, batch_size, is_distributed, testdev=False):
        ...
        valdataset = self._maybe_truncate(valdataset, "MOT20_DET_SMOKE_VAL_IMAGES")
        if is_distributed:
            batch_size = batch_size // dist.get_world_size()
