...
from .laplace_assoc import _normalize_rows, _track_history, laplace_fuse_cost
...
def _history_similarity_features(det_feat: np.ndarray, hist_feat: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    if sims.size == 0:
        ...
        return empty, empty, empty, np.zeros((0, 0), dtype=np.float32)
...
class HACAV1Checkpoint:
    ...
    def temporal_summary(self, det_feat: np.ndarray, hist_feat: np.ndarray) -> Tuple[float, float, float, float, np.ndarray]:
        sims, age_log, delta, _ = _history_similarity_features(det_feat, hist_feat)
        if sims.size == 0:
...
def haca_fuse_cost(
    ...
    selected_tracks = [tracks[idx] for idx in track_indices]
    selected_dets = [detections[idx] for idx in detection_indices]
    ...
    if valid_mask is None:
        ...
        for row, track in enumerate(selected_tracks):
            ...
            valid_mask[row] = gating_distance <= float(gating_threshold)
    valid_mask = np.asarray(valid_mask, dtype=bool)

    det_feats = []
    for det in selected_dets:
        det_feats.append(_normalize_rows(np.asarray(det.feature, dtype=np.float32))[0])
    ...
    track_gaps = np.asarray([max(0, int(getattr(track, "time_since_update", 1))) for track in selected_tracks], dtype=np.float32)
    ...
    background_gate = np.zeros((len(selected_dets),), dtype=np.float32)
    ...
    comp_margin = np.zeros((len(selected_dets),), dtype=np.float32)
    comp_entropy = np.zeros((len(selected_dets),), dtype=np.float32)
    ...
    for det_idx, det_feat in enumerate(det_feats):
        ...
        for row in rows.tolist():
            hist = _track_history(selected_tracks[row])
        ...
        std_val = max(float(np.std(anchor_vals)), 1e-6)
