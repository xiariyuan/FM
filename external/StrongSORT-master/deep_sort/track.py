...
class Track:
    ...
    def __init__(self, detection, track_id, n_init, max_age,
        ...
        self.features = []
        self.laplace_history = deque(maxlen=max(int(getattr(opt, "laplace_max_history", 30)), 1))
        self.smooth_feat = None
        self.curr_feat = None
        if feature is not None:
            feature = np.asarray(feature, dtype=np.float32)
            feature /= np.clip(np.linalg.norm(feature), 1e-12, None)
            self.features.append(feature)
            self.laplace_history.append(feature.copy())
            self.smooth_feat = feature.copy()
            self.curr_feat = feature.copy()
    ...
    def update(self, detection):
        ...
        if opt.EMA:
            ...
            self.smooth_feat = smooth_feat.copy()
        else:
            self.features.append(feature)
            self.smooth_feat = feature.copy()
