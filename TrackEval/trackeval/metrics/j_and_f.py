...
def _get_linear_sum_assignment():
    ...
    if _linear_sum_assignment is None:
        try:
            ...
            raise ImportError(
                "scipy is required for J&F metric. "
...
class JAndF(_BaseMetric):
    ...
    def eval_sequence(self, data):
        ...
        # Empty sequence guards to avoid Hungarian on empty cost matrices.
        if num_gt_ids == 0:
            return {
                'J-Mean': 0.0, 'J-Recall': 0.0, 'J-Decay': 0.0,
                'F-Mean': 0.0, 'F-Recall': 0.0, 'F-Decay': 0.0,
                'J&F': 0.0, 'num_gt_tracks': 0,
            }
        if num_tracker_ids == 0:
            return {
                'J-Mean': 0.0, 'J-Recall': 0.0, 'J-Decay': 0.0,
                'F-Mean': 0.0, 'F-Recall': 0.0, 'F-Decay': 0.0,
                'J&F': 0.0, 'num_gt_tracks': num_gt_ids,
            }
