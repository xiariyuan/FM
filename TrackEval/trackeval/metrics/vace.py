...
def _get_linear_sum_assignment():
    ...
    if _linear_sum_assignment is None:
        try:
            ...
            raise ImportError(
                "scipy is required for VACE metric. "
...
class VACE(_BaseMetric):
    ...
    def eval_sequence(self, data):
        ...
        if data['num_gt_ids'] == 0 or data['num_tracker_ids'] == 0:
            res['STDA'] = 0.0
        else:
    ...
    def _compute_final_fields(additive):
        ...
        with np.errstate(invalid='ignore'):  # Permit nan results.
            ata_denom = 0.5 * (additive['VACE_IDs'] + additive['VACE_GT_IDs'])
            if ata_denom > 0:
                final['ATA'] = additive['STDA'] / ata_denom
            else:
                final['ATA'] = 0.0
            if additive['num_non_empty_timesteps'] > 0:
