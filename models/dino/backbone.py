            ...
            for name, parameter in backbone.named_parameters():
                for keyword in backbone_freeze_keywords:
                    if keyword in name:
                        parameter.requires_grad_(False)
                        break
        if hasattr(args, "backbone_dir") and args.backbone_dir:
            ...
            if args.backbone not in PTDICT:
                raise ValueError(f"No pretrained checkpoint mapping for backbone {args.backbone}.")
