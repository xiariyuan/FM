...
def prepare_for_cdn(dn_args, training, num_queries, num_classes, hidden_dim, label_enc):
    ...
    if training:
        ...
        # 空目标保护：若整个 batch 没有任何标注，直接跳过 DN 分支
        total_labels = sum(len(t.get('labels', [])) for t in targets)
        if total_labels == 0:
        ...
        if len(known_num):
            map_known_indice = torch.cat([torch.arange(int(num), device=device) for num in known_num])  # [1,2, 1,2,3]
