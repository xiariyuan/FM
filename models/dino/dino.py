...
class DINO(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        use_dn = self.training and self.dn_number > 0 and targets is not None
        if use_dn:
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        if len(targets) == 0:
            loss_ce = src_logits.sum() * 0
            losses = {'loss_ce': loss_ce}
            if log:
                losses['class_error'] = torch.zeros([], device=src_logits.device)
            return losses

        idx = self._get_src_permutation_idx(indices, device=src_logits.device)
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        if len(targets) == 0:
            ...
            return {'loss_bbox': zero, 'loss_giou': zero, 'loss_xy': zero, 'loss_hw': zero}

        idx = self._get_src_permutation_idx(indices, device=outputs['pred_boxes'].device)
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes):
        ...
        src_idx = self._get_src_permutation_idx(indices, device=outputs["pred_masks"].device)
        tgt_idx = self._get_tgt_permutation_idx(indices, device=outputs["pred_masks"].device)
    ...
    def forward(self, outputs, targets, return_indices=False):
        ...
        """
        outputs_without_aux = {k: v for k, v in outputs.items() if k not in ['aux_outputs', 'dn_meta', 'outputs']}
        ...
        if self.training and dn_meta and 'output_known_lbs_bboxes' in dn_meta:
            ...
            for i in range(len(targets)):
                if len(targets[i]['labels']) > 0:
                    ...
                    output_idx = tgt_idx = torch.tensor([], device=device, dtype=torch.long)
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes, not_to_xyxy=False, test=False):
        ...
        total_scores = prob.shape[1] * prob.shape[2]
        k = min(num_select, total_scores)
        if k == 0:
            empty_scores = torch.empty((0,), device=out_logits.device, dtype=out_logits.dtype)
            empty_labels = torch.empty((0,), device=out_logits.device, dtype=torch.int64)
            empty_boxes = torch.empty((0, 4), device=out_bbox.device, dtype=out_bbox.dtype)
            return [{'scores': empty_scores, 'labels': empty_labels, 'boxes': empty_boxes}
                    for _ in range(out_logits.shape[0])]
