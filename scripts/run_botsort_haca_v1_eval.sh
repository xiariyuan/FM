...
SCRIPT_NAME="${SCRIPT_NAME:-scripts/run_botsort_haca_v1_eval.sh}"
VARIANT_NAME="${VARIANT_NAME:-${HACA_LABEL}_matrix}"
RUN_ROOT="${RUN_ROOT:-${REPO_ROOT}/outputs/${HACA_LABEL}_eval_${TS}}"
...
if [[ "${RUN_HEURISTIC}" == "1" ]]; then
  HEUR_EXP="mot17_frcnn_${HACA_LABEL}_heuristic_${TS}"
...
if [[ "${RUN_CURRENT_LEARNED}" == "1" ]]; then
  ...
  LEARNED_EXP="mot17_frcnn_${HACA_LABEL}_currentlearned_${TS}"
...
if [[ "${RUN_HACA}" == "1" ]]; then
  ...
  HACA_EXP="mot17_frcnn_${HACA_LABEL}_${TS}"
