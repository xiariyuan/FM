#!/usr/bin/env python3
"""ShadowPDA-V2: causal public-recovery posterior.

This file is independent from the recovered wrapper and previous pending/hold
experiments. It imports the recovered contact+identity implementation and
replaces only the low public recovery decision with a unified online posterior.
"""
...
class ShadowPDAV2Tracker(spda.ShadowPDATracker):
    def __init__(self, cfg, osr, shadow, v2: V2Config):
        super().__init__(cfg, osr, shadow)
        self.v2 = v2
        self.shadow_stats.update({
            'v2_public_candidates': 0,
            'v2_public_recovered': 0,
            'v2_public_blocked': 0,
        })
    ...
    def _public_score(self, st, det, lost_age: int, lost_app: float, identity_margin: float) -> tuple[float, dict[str, float]]:
        z = {
            'z_reliability': clip01(float(st.reliability)),
            'z_identity': max(-1.0, min(1.0, float(identity_margin) / max(1e-6, float(self.v2.id_scale)))),
            'z_app': clip01(float(lost_app)),
            'z_det': clip01(float(det.score)),
            'z_contact': clip01(float(st.active_overlap)),
            'z_temporal': self._temporal_score(st),
        ...
        score = (
            ...
            + float(self.v2.w_identity) * z['z_identity']
            + float(self.v2.w_app) * z['z_app']
            + float(self.v2.w_det) * z['z_det']
            + float(self.v2.w_contact) * z['z_contact']
    ...
    def _try_ultra_low_recover(self, lost_tracks: Sequence[spda.base.DMMTrack], low_dets: Sequence[spda.base.DMMTrack], active_tracks: Sequence[spda.base.DMMTrack] | None = None) -> List[Tuple[int, spda.base.DMMTrack]]:
        ...
        for idx, track in enumerate(lost_tracks):
            ...
            self.shadow_stats['v2_public_candidates'] += 1
            ...
            public_score, z = self._public_score(st, det, lost_age, lost_app, identity_margin)
            ...
            if ok:
                ...
                self.shadow_stats['v2_public_recovered'] += 1
            else:
                self.shadow_stats['ultra_low_blocked'] += 1
                self.shadow_stats['v2_public_blocked'] += 1
        return out


def parse_args() -> tuple[argparse.Namespace, V2Config]:
    v2_parser = argparse.ArgumentParser(add_help=False)
    v2_parser.add_argument('--v2-theta-public', type=float, default=2.0)
    ...
    v2_parser.add_argument('--v2-w-contact', type=float, default=0.8)
    ...
    v2 = V2Config(
        ...
        w_contact=float(v2_args.v2_w_contact),
...
def make_tracker(args: argparse.Namespace, v2: V2Config) -> ShadowPDAV2Tracker:
    ...
    return ShadowPDAV2Tracker(cfg, osr, shadow, v2)
