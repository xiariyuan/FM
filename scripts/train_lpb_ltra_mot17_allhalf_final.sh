...
OUT_ROOT="${OUT_ROOT:-${REPO_ROOT}/outputs/lpb_ltra_allhalf_final_${TS}}"
OUT_ROOT="$(realpath -m "${OUT_ROOT}")"
OUT_NPZ="${OUT_NPZ:-${OUT_ROOT}/mot17_lpb_ltra_allhalf_${TS}.npz}"
...
update_plan_status() {
  ...
  "${PYTHON_BIN}" "${REPO_ROOT}/scripts/upsert_experiment_plan.py" \
    ...
    --status "${status}" \
    --kind train \
    --script "scripts/train_lpb_ltra_mot17_allhalf_final.sh" \
    --dataset MOT17 \
    --split all_half_final \
    --tracker-family BoT-SORT \
    --variant lpb_ltra_allhalf_final \
    --run-root "${OUT_ROOT}" \
    --checkpoint "${OUT_NPZ}" \
    --log-path "${LOG_PATH}" \
    --extra "${extras[@]}"
...
ALL_NPZS=("${TRAIN_NPZS[@]}" "${VAL_NPZS[@]}")
if [[ ${#ALL_NPZS[@]} -eq 0 ]]; then
  echo "[error] no NPZ shards found under ${DATA_ROOT}"
  exit 1
fi

echo "[start] $(date '+%F %T %z')"
echo "[allhalf_shards] ${#ALL_NPZS[@]}"
printf '  %s\n' "${ALL_NPZS[@]}"
