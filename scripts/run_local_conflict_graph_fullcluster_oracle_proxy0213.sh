...
OUT_DIR="${1:-${REPO_ROOT}/outputs/local_conflict_graph_fullcluster_oracle_proxy0213_$(date +%Y%m%d_%H%M%S)}"
...
row = {
    "exp_name": "local_conflict_graph_fullcluster_oracle_proxy0213",
...
overrides = {
    ...
    "ASSOC_LOCAL_CONFLICT_GRAPH_MODE": "oracle_full",
...
doc = {
    "description": "Local conflict graph full-cluster replacement oracle on proxy0213.",
    "manifest": {
...
"${PYTHON_BIN}" "${REPO_ROOT}/scripts/append_experiment_record.py" \
  ...
  --script "scripts/run_local_conflict_graph_fullcluster_oracle_proxy0213.sh" \
  --dataset "MOT17" \
  --split "val0213_proxy" \
  --tracker-family "ByteTrack" \
  --variant "local_conflict_graph_fullcluster_oracle_proxy0213" \
  --tag "local_conflict_graph_mainline" \
  --run-root "${OUT_DIR}" \
  --summary-csv "${SUMMARY_CSV}" \
  --checkpoint "${BASE_CKPT}" \
  --log-path "${LOG_PATH}" \
  --notes "local conflict graph full cluster replacement oracle on proxy0213" \
  --extra "oracle_group_jsonl=${ORACLE_JSONL}" "graph_mode=oracle_full" "graph_topk=${TOPK}" "graph_min_detections=${MIN_DETECTIONS}"

if ! "${PYTHON_BIN}" "${REPO_ROOT}/scripts/post_experiment_pro_bundle.py" \
  --run-root "${OUT_DIR}" \
  --tag "local_conflict_graph_fullcluster_oracle_bundle" \
  --label "local_conflict_graph_fullcluster_oracle_proxy0213" \
  --status "$( [[ "${STATUS}" == "success" ]] && echo ok || echo failed )"; then
  echo "[local-conflict-graph-fullcluster-oracle] warning: failed to build Pro review bundle for ${OUT_DIR}" >&2
fi

echo "[local-conflict-graph-fullcluster-oracle] status=${STATUS} out_dir=${OUT_DIR}"
