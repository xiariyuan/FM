...
import os
# Required before importing torch or initializing CUDA/CuBLAS.
...
ROOT = Path("outputs/mot20_m23_20260718/m23_59_relation_pretrained_hierarchical_flow_v2")
EXTERNAL_MANIFEST = ROOT / "external_dataset_manifest.json"
PREREG_DOC = Path("docs/m23_59_relation_pretrained_hierarchical_flow_prereg_v2_20260720.md")
...
def class_balanced_focal_per_example(logits: torch.Tensor, target: torch.Tensor, gamma: float = 2.0) -> torch.Tensor:
    """Deterministic class-balanced focal loss, retaining one value per sample."""
    target = target.float()
    pos = target.sum(); neg = target.numel() - pos
    pos_weight = (neg / pos.clamp_min(1.0)).clamp(1.0, 1000.0)
    bce = F.binary_cross_entropy_with_logits(logits, target, reduction="none", pos_weight=pos_weight)
...
def calibrated_risk_threshold(error: np.ndarray, score: np.ndarray, limit: float) -> dict[str, Any]:
    """Largest score cutoff whose selected-set one-sided CP UCB satisfies the fixed limit."""
    error = np.asarray(error, int); score = np.asarray(score, float)
    finite = np.isfinite(score); error = error[finite]; score = score[finite]
    if not len(score):
        return {"risk_limit": limit, "score_threshold": -1.0, "selected": 0, "errors": 0,
                "empirical_error_rate": None, "clopper_pearson_upper_95": 1.0}
    order = np.lexsort((np.arange(len(score)), score)); error = error[order]; score = score[order]
    cumulative = np.cumsum(error); best = None; end = 0
    while end < len(score):
        value = score[end]; next_end = end + 1
        while next_end < len(score) and score[next_end] == value: next_end += 1
        selected = next_end; errors = int(cumulative[next_end - 1]); ucb = clopper_pearson_upper(errors, selected)
        if ucb <= limit: best = (float(value), selected, errors, ucb)
        end = next_end
    if best is None:
        return {"risk_limit": limit, "score_threshold": -1.0, "selected": 0, "errors": 0,
                "empirical_error_rate": None, "clopper_pearson_upper_95": 1.0}
    threshold, selected, errors, ucb = best
    return {"risk_limit": limit, "score_threshold": threshold, "selected": selected, "errors": errors,
            "empirical_error_rate": errors / selected, "clopper_pearson_upper_95": ucb}
...
def build_external_examples(split: str) -> dict[str, Any]:
    ...
    rel_out_neg: list[np.ndarray] = []; rel_out_neg_m: list[np.ndarray] = []
    rel_in_neg: list[np.ndarray] = []; rel_in_neg_m: list[np.ndarray] = []
    rel_group: list[int] = []; rel_bucket: list[int] = []
    pair_src1: list[np.ndarray] = []; pair_src1_m: list[np.ndarray] = []
    pair_pos1: list[np.ndarray] = []; pair_pos1_m: list[np.ndarray] = []
    pair_src2: list[np.ndarray] = []; pair_src2_m: list[np.ndarray] = []
    pair_pos2: list[np.ndarray] = []; pair_pos2_m: list[np.ndarray] = []
    pair_group: list[int] = []; pair_bucket: list[int] = []
    seq_reports = {}

    def seq_stack(values: list[np.ndarray], length: int, dtype: np.dtype) -> np.ndarray:
        if values:
            return np.asarray(values, dtype)
        return np.zeros((0, length, APP_DIM + GEOM_DIM), dtype)

    def mask_stack(values: list[np.ndarray], length: int) -> np.ndarray:
        if values:
            return np.asarray(values, np.uint8)
        return np.zeros((0, length), np.uint8)
    ...
    for group, seq in enumerate(sequences):
        ...
        pure_ids = np.unique(np.linspace(0, len(windows) - 1, min(PURE_PER_SEQ, len(windows)), dtype=int))
        partner_cache: dict[int, int | None] = {}
        counts = defaultdict(int)
        for wi in pure_ids:
            w = windows[int(wi)]
            x, m = pad_sequence(w.indices, app, geom, MAX_NODE_ROWS)
            node_x.append(x); node_m.append(m); node_y.append(0)
            boundary_y.append(np.zeros(MAX_NODE_ROWS - 1, np.int8))
            node_group.append(group); node_type.append(0); aba_event.append(0); counts["pure"] += 1
        candidate_ids = np.unique(np.linspace(0, len(windows) - 1, min(max(AB_PER_SEQ, ABA_PER_SEQ), len(windows)), dtype=int))
        ...
        for wi in candidate_ids[:AB_PER_SEQ]:
            ...
            x1, m1 = pad_sequence(a.indices[:15], app, geom, 15)
            x2, m2 = pad_sequence(b.indices[-15:], app, geom, 15)
            by = np.zeros(29, np.int8); by[14] = 1
            node_x.append(np.concatenate([x1, x2])); node_m.append(np.concatenate([m1, m2]))
            node_y.append(1); boundary_y.append(by); node_group.append(group); node_type.append(1); aba_event.append(0)
            counts["A_to_B"] += 1
        ...
        # (identity, row indices, start frame, end frame, appearance prototype, endpoint geometry)
        segment_bank: list[tuple[int, tuple[int, ...], int, int, np.ndarray, np.ndarray]] = []
        for ident in sorted(set(map(int, identities))):
            ids = np.flatnonzero(identities == ident).tolist(); ids.sort(key=lambda i: (frames[i], i))
            if len(ids) < SEG_ROWS: continue
            starts = list(range(0, len(ids) - SEG_ROWS + 1, SEG_ROWS))
            if starts and starts[-1] != len(ids) - SEG_ROWS: starts.append(len(ids) - SEG_ROWS)
            for st in starts:
                q = tuple(map(int, ids[st:st + SEG_ROWS]))
                proto = app[np.asarray(q)].mean(axis=0); proto /= max(float(np.linalg.norm(proto)), 1e-12)
                segment_bank.append((ident, q, int(frames[q[0]]), int(frames[q[-1]]), proto, geom[q[-1], :6].copy()))
        ...
        true_relations: list[tuple[int, int, int]] = []
        for bucket_index, (bucket_name, lo, hi) in enumerate(GAP_BUCKETS):
            candidates: list[tuple[int, int]] = []
            for ident, ids in sorted(by_identity.items()):
                for ii in range(len(ids)):
                    aidx = ids[ii]; aend = segment_bank[aidx][3]
                    for jj in range(ii + 1, len(ids)):
                        bidx = ids[jj]; gap = segment_bank[bidx][2] - aend - 1
                        if gap < lo: continue
                        if gap > hi: break
                        candidates.append((aidx, bidx)); break
            if len(candidates) > RELATION_PER_BUCKET_PER_SEQ:
                keep = np.unique(np.linspace(0, len(candidates) - 1, RELATION_PER_BUCKET_PER_SEQ, dtype=int))
                candidates = [candidates[int(i)] for i in keep]
            for aidx, bidx in candidates:
                a = segment_bank[aidx]; b = segment_bank[bidx]
                out_hard: list[tuple[float, int]] = []
                in_hard: list[tuple[float, int]] = []
                for ni, n in enumerate(segment_bank):
                    if n[0] == a[0]: continue
                    if abs(n[2] - b[2]) <= 30:
                        gd = float(np.linalg.norm(b[5][:4] - n[5][:4]))
                        if gd <= 3.0:
                            out_hard.append((0.70 * float(a[4] @ n[4]) + 0.30 * math.exp(-gd), ni))
                    ngap = b[2] - n[3] - 1
                    if lo <= ngap <= hi and abs(n[3] - a[3]) <= 30:
                        gd = float(np.linalg.norm(a[5][:4] - n[5][:4]))
                        if gd <= 3.0:
                            in_hard.append((0.70 * float(n[4] @ b[4]) + 0.30 * math.exp(-gd), ni))
                if not out_hard or not in_hard: continue
                out_hard.sort(key=lambda z: (-z[0], segment_bank[z[1]][0], segment_bank[z[1]][2], z[1]))
                in_hard.sort(key=lambda z: (-z[0], segment_bank[z[1]][0], segment_bank[z[1]][2], z[1]))
                on = segment_bank[out_hard[0][1]]; inn = segment_bank[in_hard[0][1]]
                sx, sm = pad_sequence(a[1], app, geom, SEG_ROWS); px, pm = pad_sequence(b[1], app, geom, SEG_ROWS)
                ox, om = pad_sequence(on[1], app, geom, SEG_ROWS); ix, im = pad_sequence(inn[1], app, geom, SEG_ROWS)
                rel_src.append(sx); rel_src_m.append(sm); rel_pos.append(px); rel_pos_m.append(pm)
                rel_out_neg.append(ox); rel_out_neg_m.append(om); rel_in_neg.append(ix); rel_in_neg_m.append(im)
                rel_group.append(group); rel_bucket.append(bucket_index); true_relations.append((bucket_index, aidx, bidx))
                counts[f"gap_{bucket_name}"] += 1
        # Paired replacement is a joint comparison of two true relations against both crossed relations.
        true_relations.sort(key=lambda z: (z[0], segment_bank[z[2]][2], segment_bank[z[1]][0], segment_bank[z[1]][2]))
        used_pairs = 0
        for i, (bucket_index, aidx, bidx) in enumerate(true_relations):
            if used_pairs >= SWAP_PER_SEQ: break
            a = segment_bank[aidx]; b = segment_bank[bidx]
            chosen = None
            for bucket2, cidx, didx in true_relations[i + 1:]:
                if bucket2 != bucket_index: continue
                c = segment_bank[cidx]; d = segment_bank[didx]
                if c[0] == a[0] or abs(d[2] - b[2]) > 30: continue
                if a[3] >= d[2] or c[3] >= b[2]: continue
                chosen = (c, d); break
            if chosen is None: continue
            c, d = chosen
            ax, am = pad_sequence(a[1], app, geom, SEG_ROWS); bx, bm = pad_sequence(b[1], app, geom, SEG_ROWS)
            cx, cm = pad_sequence(c[1], app, geom, SEG_ROWS); dx, dm = pad_sequence(d[1], app, geom, SEG_ROWS)
            pair_src1.append(ax); pair_src1_m.append(am); pair_pos1.append(bx); pair_pos1_m.append(bm)
            pair_src2.append(cx); pair_src2_m.append(cm); pair_pos2.append(dx); pair_pos2_m.append(dm)
            pair_group.append(group); pair_bucket.append(bucket_index); used_pairs += 1; counts["paired_replacement"] += 1
    ...
    arrays = {
        ...
        "rel_src": seq_stack(rel_src, SEG_ROWS, np.float16), "rel_src_mask": mask_stack(rel_src_m, SEG_ROWS),
        "rel_pos": seq_stack(rel_pos, SEG_ROWS, np.float16), "rel_pos_mask": mask_stack(rel_pos_m, SEG_ROWS),
        "rel_out_neg": seq_stack(rel_out_neg, SEG_ROWS, np.float16), "rel_out_neg_mask": mask_stack(rel_out_neg_m, SEG_ROWS),
        "rel_in_neg": seq_stack(rel_in_neg, SEG_ROWS, np.float16), "rel_in_neg_mask": mask_stack(rel_in_neg_m, SEG_ROWS),
        "rel_group": np.asarray(rel_group, np.int16), "rel_bucket": np.asarray(rel_bucket, np.int8),
        "pair_src1": seq_stack(pair_src1, SEG_ROWS, np.float16), "pair_src1_mask": mask_stack(pair_src1_m, SEG_ROWS),
        "pair_pos1": seq_stack(pair_pos1, SEG_ROWS, np.float16), "pair_pos1_mask": mask_stack(pair_pos1_m, SEG_ROWS),
        "pair_src2": seq_stack(pair_src2, SEG_ROWS, np.float16), "pair_src2_mask": mask_stack(pair_src2_m, SEG_ROWS),
        "pair_pos2": seq_stack(pair_pos2, SEG_ROWS, np.float16), "pair_pos2_mask": mask_stack(pair_pos2_m, SEG_ROWS),
        "pair_group": np.asarray(pair_group, np.int16), "pair_bucket": np.asarray(pair_bucket, np.int8),
    }
    artifacts = save_array_set(out, arrays)
    report = {
        "experiment": "M23-59 v2 relation-specific hard corruption examples", "version": "v2", "split": split,
        "physical_sequences": sequences, "external_supervision": True, "v1_artifact_reuse": False,
        "node_examples": len(node_x), "relation_triplets": len(rel_src), "paired_replacements": len(pair_group),
        "synthetic_protocol": {
            ...
            "paired_replacement_per_sequence_cap": SWAP_PER_SEQ, "gap_buckets": GAP_BUCKETS,
            "outgoing_hard_negative": "different identity; destination start within 30 frames; geometry<=3; rank 0.70 source-to-destination appearance +0.30 geometry",
            "incoming_hard_negative": "different identity predecessor; predecessor end within 30 frames of true predecessor; same gap bucket; geometry<=3; rank 0.70 predecessor-to-destination appearance +0.30 geometry",
            "paired_replacement": "two true same-bucket relations with destination starts within 30 frames; compare true joint energy against both crossed time-forward relations",
        },
...
def training_objective(model: HierarchicalRelationEncoder, data: dict[str, np.ndarray],
                       node_ids: np.ndarray, relation_ids: np.ndarray, pair_ids: np.ndarray,
                       dev: torch.device, q_node: torch.Tensor, q_relation: torch.Tensor
                       ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, dict[str, float]]:
    x = tensor(data["node_x"][node_ids], dev); m = tensor(data["node_mask"][node_ids], dev)
    ny = tensor(data["node_label"][node_ids], dev); by = tensor(data["boundary_label"][node_ids], dev)
    ng = tensor(data["node_group"][node_ids], dev, torch.long)
    nl, bl, bv = model.node_and_boundary(x, m)
    node_per = class_balanced_focal_per_example(nl, ny)
    node_base = node_per.mean(); node_dro, q_node = group_weighted(node_per, ng, q_node)
    valid_boundary = (bv > 0) & (by >= 0) & (ny[:, None] > 0)
    boundary = focal_loss(bl, by.clamp_min(0), valid_boundary)
    count = ((torch.sigmoid(bl) * bv).sum(dim=1) - by.clamp_min(0).sum(dim=1)).abs().mean()
    sparsity = (torch.sigmoid(bl) * bv).mean()

    sx = tensor(data["rel_src"][relation_ids], dev); sm = tensor(data["rel_src_mask"][relation_ids], dev)
    px = tensor(data["rel_pos"][relation_ids], dev); pm = tensor(data["rel_pos_mask"][relation_ids], dev)
    ox = tensor(data["rel_out_neg"][relation_ids], dev); om = tensor(data["rel_out_neg_mask"][relation_ids], dev)
    ix = tensor(data["rel_in_neg"][relation_ids], dev); im = tensor(data["rel_in_neg_mask"][relation_ids], dev)
    rg = tensor(data["rel_group"][relation_ids], dev, torch.long)
    ps, pr = model.relation(sx, sm, px, pm)
    oscore, orisk = model.relation(sx, sm, ox, om)
    iscore, irisk = model.relation(ix, im, px, pm)
    outgoing_per = F.softplus(-(ps - oscore)); incoming_per = F.softplus(-(ps - iscore))
    relation_per = 0.5 * (outgoing_per + incoming_per)
    relation_dro, q_relation = group_weighted(relation_per, rg, q_relation)
    outgoing = outgoing_per.mean(); incoming = incoming_per.mean()
    risk_terms = [
        F.binary_cross_entropy_with_logits(pr, torch.zeros_like(pr)),
        F.binary_cross_entropy_with_logits(orisk, torch.ones_like(orisk)),
        F.binary_cross_entropy_with_logits(irisk, torch.ones_like(irisk)),
    ]
    paired = ps.sum() * 0.0
    if len(pair_ids):
        a = tensor(data["pair_src1"][pair_ids], dev); am = tensor(data["pair_src1_mask"][pair_ids], dev)
        b = tensor(data["pair_pos1"][pair_ids], dev); bm = tensor(data["pair_pos1_mask"][pair_ids], dev)
        c = tensor(data["pair_src2"][pair_ids], dev); cm = tensor(data["pair_src2_mask"][pair_ids], dev)
        d = tensor(data["pair_pos2"][pair_ids], dev); dm = tensor(data["pair_pos2_mask"][pair_ids], dev)
        ab, rab = model.relation(a, am, b, bm); cd, rcd = model.relation(c, cm, d, dm)
        ad, rad = model.relation(a, am, d, dm); cb, rcb = model.relation(c, cm, b, bm)
        paired = F.softplus(-((ab + cd) - (ad + cb))).mean()
        risk_terms.extend([
            F.binary_cross_entropy_with_logits(rab, torch.zeros_like(rab)),
            F.binary_cross_entropy_with_logits(rcd, torch.zeros_like(rcd)),
            F.binary_cross_entropy_with_logits(rad, torch.ones_like(rad)),
            F.binary_cross_entropy_with_logits(rcb, torch.ones_like(rcb)),
        ])
    risk = torch.stack(risk_terms).mean()
    group_dro_regularizer = 0.5 * (node_dro + relation_dro)
    total = (
        LOSS_WEIGHTS["node_focal"] * node_base
        + LOSS_WEIGHTS["conditional_boundary_focal"] * boundary
        + LOSS_WEIGHTS["boundary_count_consistency"] * count
        + LOSS_WEIGHTS["outgoing_listwise"] * outgoing
        + LOSS_WEIGHTS["incoming_listwise"] * incoming
        + LOSS_WEIGHTS["paired_replacement"] * paired
        + LOSS_WEIGHTS["catastrophic_risk"] * risk
        + LOSS_WEIGHTS["sequence_group_dro"] * group_dro_regularizer
        + LOSS_WEIGHTS["edit_sparsity_source_anchor"] * sparsity
    )
    parts = {"node": float(node_base.detach().cpu()), "boundary": float(boundary.detach().cpu()),
             "count": float(count.detach().cpu()), "outgoing": float(outgoing.detach().cpu()),
             "incoming": float(incoming.detach().cpu()), "paired": float(paired.detach().cpu()),
             "risk": float(risk.detach().cpu()), "group_dro": float(group_dro_regularizer.detach().cpu()),
             "sparsity": float(sparsity.detach().cpu()), "total": float(total.detach().cpu())}
    return total, q_node, q_relation, parts
...
def validation_metrics(model: HierarchicalRelationEncoder, data: dict[str, np.ndarray], dev: torch.device) -> dict[str, Any]:
    model.eval(); node_scores = []; node_y = []; boundary_scores = []; boundary_y = []; aba_hit = []
    with torch.no_grad():
        for ids in [np.arange(i, min(i + 1024, len(data["node_label"]))) for i in range(0, len(data["node_label"]), 1024)]:
            nl, bl, valid = model.node_and_boundary(tensor(data["node_x"][ids], dev), tensor(data["node_mask"][ids], dev))
            node_scores.append(torch.sigmoid(nl).cpu().numpy()); node_y.append(np.asarray(data["node_label"][ids]))
            by = np.asarray(data["boundary_label"][ids]); conditional = (np.asarray(data["node_label"][ids])[:, None] > 0) & (valid.cpu().numpy() > 0)
            ...
            for local, idx in enumerate(ids):
                if int(data["aba_event"][idx]) > 0:
                    aba_hit.append(int(set(np.argsort(-probs[local])[:2].tolist()) == {9, 19}))
        true_s = []; out_s = []; in_s = []; true_r = []; out_r = []; in_r = []
        for ids in [np.arange(i, min(i + 1024, len(data["rel_group"]))) for i in range(0, len(data["rel_group"]), 1024)]:
            ps, pr = model.relation(tensor(data["rel_src"][ids], dev), tensor(data["rel_src_mask"][ids], dev), tensor(data["rel_pos"][ids], dev), tensor(data["rel_pos_mask"][ids], dev))
            os, ora = model.relation(tensor(data["rel_src"][ids], dev), tensor(data["rel_src_mask"][ids], dev), tensor(data["rel_out_neg"][ids], dev), tensor(data["rel_out_neg_mask"][ids], dev))
            ins, ira = model.relation(tensor(data["rel_in_neg"][ids], dev), tensor(data["rel_in_neg_mask"][ids], dev), tensor(data["rel_pos"][ids], dev), tensor(data["rel_pos_mask"][ids], dev))
            true_s.append(ps.cpu().numpy()); out_s.append(os.cpu().numpy()); in_s.append(ins.cpu().numpy())
            true_r.append(torch.sigmoid(pr).cpu().numpy()); out_r.append(torch.sigmoid(ora).cpu().numpy()); in_r.append(torch.sigmoid(ira).cpu().numpy())
        pair_true = []; pair_swap = []
        for ids in [np.arange(i, min(i + 1024, len(data["pair_group"]))) for i in range(0, len(data["pair_group"]), 1024)]:
            ab, _ = model.relation(tensor(data["pair_src1"][ids], dev), tensor(data["pair_src1_mask"][ids], dev), tensor(data["pair_pos1"][ids], dev), tensor(data["pair_pos1_mask"][ids], dev))
            cd, _ = model.relation(tensor(data["pair_src2"][ids], dev), tensor(data["pair_src2_mask"][ids], dev), tensor(data["pair_pos2"][ids], dev), tensor(data["pair_pos2_mask"][ids], dev))
            ad, _ = model.relation(tensor(data["pair_src1"][ids], dev), tensor(data["pair_src1_mask"][ids], dev), tensor(data["pair_pos2"][ids], dev), tensor(data["pair_pos2_mask"][ids], dev))
            cb, _ = model.relation(tensor(data["pair_src2"][ids], dev), tensor(data["pair_src2_mask"][ids], dev), tensor(data["pair_pos1"][ids], dev), tensor(data["pair_pos1_mask"][ids], dev))
            pair_true.append((ab + cd).cpu().numpy()); pair_swap.append((ad + cb).cpu().numpy())
    ny = np.concatenate(node_y); ns = np.concatenate(node_scores)
    by = np.concatenate(boundary_y) if boundary_y else np.zeros(0, int); bs = np.concatenate(boundary_scores) if boundary_scores else np.zeros(0, float)
    ps = np.concatenate(true_s); oscore = np.concatenate(out_s); iscore = np.concatenate(in_s)
    pr = np.concatenate(true_r); ora = np.concatenate(out_r); ira = np.concatenate(in_r)
    outgoing_r1 = float((ps > oscore).mean()); incoming_r1 = float((ps > iscore).mean())
    paired_r1 = float((np.concatenate(pair_true) > np.concatenate(pair_swap)).mean()) if pair_true else 0.0
    false_events = [oscore >= ps, iscore >= ps]
    if pair_true: false_events.append(np.concatenate(pair_swap) >= np.concatenate(pair_true))
    catastrophic_false_link = float(np.concatenate(false_events).mean())
    risk_y = np.concatenate([np.zeros(len(pr), int), np.ones(len(ora), int), np.ones(len(ira), int)])
    risk_s = np.concatenate([pr, ora, ira])
    node_report = summarize_binary(ny, ns); boundary_report = summarize_binary(by, bs); risk_report = summarize_binary(risk_y, risk_s)
    relation_r1 = 0.5 * (outgoing_r1 + incoming_r1)
    composite = 0.30 * float(node_report["pr_auc"] or 0.0) + 0.35 * float(boundary_report["pr_auc"] or 0.0) + 0.25 * relation_r1 + 0.10 * (1.0 - catastrophic_false_link)
    return {"node": node_report, "conditional_boundary": boundary_report,
            "A_to_B_to_A_exact_two_boundary_recall": float(np.mean(aba_hit)) if aba_hit else 0.0,
            "outgoing_successor_R_at_1_pairwise": outgoing_r1, "incoming_predecessor_R_at_1_pairwise": incoming_r1,
            "paired_replacement_R_at_1": paired_r1, "relation_R_at_1_pairwise_mean": relation_r1,
            "catastrophic_false_link_rate": catastrophic_false_link, "risk": risk_report,
            "checkpoint_selection_composite": composite}


def train_external_seed(seed: int, train: dict[str, np.ndarray], val: dict[str, np.ndarray]) -> dict[str, Any]:
    set_determinism(seed); dev = device(); model = HierarchicalRelationEncoder().to(dev)
    params = parameter_count(model)
    if params > PARAMETER_CAP: raise RuntimeError(f"parameter cap exceeded: {params}")
    ...
    rng = np.random.default_rng(seed); seed_root = ROOT / "external_pretraining" / f"seed_{seed}"
    seed_root.mkdir(parents=True, exist_ok=True); best = None; history = []
    for epoch in range(EXTERNAL_EPOCHS):
        model.train(); epoch_parts: list[dict[str, float]] = []
        nb = list(sample_indices(len(train["node_label"]), BATCH_NODES, rng))
        rb = list(sample_indices(len(train["rel_group"]), BATCH_RELATIONS, rng))
        pb = list(sample_indices(len(train["pair_group"]), BATCH_RELATIONS, rng)) if len(train["pair_group"]) else [np.zeros(0, np.int64)]
        for step in range(max(len(nb), len(rb), len(pb))):
            loss, q_node, q_rel, parts = training_objective(model, train, nb[step % len(nb)], rb[step % len(rb)], pb[step % len(pb)], dev, q_node, q_rel)
            if not torch.isfinite(loss): raise RuntimeError(f"non-finite external loss seed={seed} epoch={epoch+1}")
            optimizer.zero_grad(set_to_none=True); loss.backward(); nn.utils.clip_grad_norm_(model.parameters(), 5.0); optimizer.step(); epoch_parts.append(parts)
        metrics = validation_metrics(model, val, dev)
        means = {f"train_{k}": float(np.mean([p[k] for p in epoch_parts])) for k in epoch_parts[0]}
        row = {"epoch": epoch + 1, **means, **metrics}; history.append(row)
        candidate = (float(metrics["checkpoint_selection_composite"]), -(epoch + 1))
        if best is None or candidate > best[0]:
            ckpt = seed_root / "best.pt"
            torch.save({"model": model.state_dict(), "seed": seed, "epoch": epoch + 1, "parameter_count": params, "validation": metrics}, ckpt)
            best = (candidate, ckpt, row)
    history_path = seed_root / "history.json"; json_write(history_path, history); assert best is not None
    report = {"seed": seed, "epochs": EXTERNAL_EPOCHS, "parameter_count": params, "best_epoch": best[2]["epoch"],
              "validation": best[2], "checkpoint": str(best[1]), "checkpoint_sha256": sha256_file(best[1]),
              "history": str(history_path), "history_sha256": sha256_file(history_path)}
    json_write(seed_root / "report.json", report); return report
...
def mot20_relation_triplets(seq: str, cap: int = RELATION_TRAIN_CAP_PER_MOT20_SEQ) -> dict[str, np.ndarray]:
    ...
    true_rows: list[tuple[int, int, int, int]] = []
    incoming_by_dst = {int(dst): g.copy() for dst, g in teacher.groupby("dst_chunk", sort=True)}
    for src, group in teacher.groupby("src_chunk", sort=True):
        ...
        positive.sort_values(["teacher_identity_order_delta", "dst_chunk"], kind="mergesort", inplace=True); pos = positive.iloc[0]
        outgoing = group[group.teacher_same_identity_forward <= 0].copy()
        incoming = incoming_by_dst.get(int(pos.dst_chunk), pd.DataFrame()).copy()
        if incoming.empty: continue
        incoming = incoming[incoming.teacher_same_identity_forward <= 0]
        if outgoing.empty or incoming.empty: continue
        outgoing.sort_values(["appearance_cos", "motion_error_min", "dst_chunk"], ascending=[False, True, True], kind="mergesort", inplace=True)
        incoming.sort_values(["appearance_cos", "motion_error_min", "src_chunk"], ascending=[False, True, True], kind="mergesort", inplace=True)
        outneg = outgoing.iloc[0]; inneg = incoming.iloc[0]
        gt = int(pos.teacher_src_dominant_gt)
        rows.append((int(src), int(pos.dst_chunk), int(outneg.dst_chunk), int(inneg.src_chunk), int(pos.gap), gt))
        true_rows.append((int(src), int(pos.dst_chunk), int(pos.gap), gt))
    if len(rows) > cap:
        keep = np.unique(np.linspace(0, len(rows) - 1, cap, dtype=int)); rows = [rows[int(i)] for i in keep]
    dim = APP_DIM + GEOM_DIM
    def seq(ids: list[int]) -> np.ndarray:
        return x[np.asarray(ids, np.int64)] if ids else np.zeros((0, MAX_NODE_ROWS, dim), np.float16)
    def mseq(ids: list[int]) -> np.ndarray:
        return mask[np.asarray(ids, np.int64)] if ids else np.zeros((0, MAX_NODE_ROWS), np.uint8)
    src = [r[0] for r in rows]; pos = [r[1] for r in rows]; outneg = [r[2] for r in rows]; inneg = [r[3] for r in rows]
    pairs: list[tuple[int, int, int, int]] = []
    true_rows.sort(key=lambda r: (r[2], int(nodes.iloc[r[1]].first_frame), r[3], r[0], r[1]))
    for i, (a, b, gap, gt) in enumerate(true_rows):
        if len(pairs) >= SWAP_PER_SEQ: break
        for c, d, gap2, gt2 in true_rows[i + 1:]:
            if gt2 == gt: continue
            if abs(int(nodes.iloc[d].first_frame) - int(nodes.iloc[b].first_frame)) > 30: continue
            if int(nodes.iloc[a].last_frame) >= int(nodes.iloc[d].first_frame): continue
            if int(nodes.iloc[c].last_frame) >= int(nodes.iloc[b].first_frame): continue
            pairs.append((a, b, c, d)); break
    return {
        "rel_src": seq(src), "rel_src_mask": mseq(src), "rel_pos": seq(pos), "rel_pos_mask": mseq(pos),
        "rel_out_neg": seq(outneg), "rel_out_neg_mask": mseq(outneg), "rel_in_neg": seq(inneg), "rel_in_neg_mask": mseq(inneg),
        "rel_gap": np.asarray([r[4] for r in rows], np.int16),
        "pair_src1": seq([p[0] for p in pairs]), "pair_src1_mask": mseq([p[0] for p in pairs]),
        "pair_pos1": seq([p[1] for p in pairs]), "pair_pos1_mask": mseq([p[1] for p in pairs]),
        "pair_src2": seq([p[2] for p in pairs]), "pair_src2_mask": mseq([p[2] for p in pairs]),
        "pair_pos2": seq([p[3] for p in pairs]), "pair_pos2_mask": mseq([p[3] for p in pairs]),
    }


def calibrate_model_risk(model: HierarchicalRelationEncoder, node_data: list[dict[str, np.ndarray]],
                         relation_data: dict[str, np.ndarray], node_threshold: float,
                         boundary_threshold: float, dev: torch.device) -> dict[str, Any]:
    model.eval(); link_error = []; link_score = []
    with torch.no_grad():
        for start in range(0, len(relation_data["rel_group"]), 1024):
            ids = np.arange(start, min(start + 1024, len(relation_data["rel_group"])))
            _, tr = model.relation(tensor(relation_data["rel_src"][ids], dev), tensor(relation_data["rel_src_mask"][ids], dev), tensor(relation_data["rel_pos"][ids], dev), tensor(relation_data["rel_pos_mask"][ids], dev))
            _, ora = model.relation(tensor(relation_data["rel_src"][ids], dev), tensor(relation_data["rel_src_mask"][ids], dev), tensor(relation_data["rel_out_neg"][ids], dev), tensor(relation_data["rel_out_neg_mask"][ids], dev))
            _, ira = model.relation(tensor(relation_data["rel_in_neg"][ids], dev), tensor(relation_data["rel_in_neg_mask"][ids], dev), tensor(relation_data["rel_pos"][ids], dev), tensor(relation_data["rel_pos_mask"][ids], dev))
            link_error.extend([np.zeros(len(ids), int), np.ones(len(ids), int), np.ones(len(ids), int)])
            link_score.extend([torch.sigmoid(tr).cpu().numpy(), torch.sigmoid(ora).cpu().numpy(), torch.sigmoid(ira).cpu().numpy()])
        for start in range(0, len(relation_data["pair_group"]), 1024):
            ids = np.arange(start, min(start + 1024, len(relation_data["pair_group"])))
            _, rab = model.relation(tensor(relation_data["pair_src1"][ids], dev), tensor(relation_data["pair_src1_mask"][ids], dev), tensor(relation_data["pair_pos1"][ids], dev), tensor(relation_data["pair_pos1_mask"][ids], dev))
            _, rcd = model.relation(tensor(relation_data["pair_src2"][ids], dev), tensor(relation_data["pair_src2_mask"][ids], dev), tensor(relation_data["pair_pos2"][ids], dev), tensor(relation_data["pair_pos2_mask"][ids], dev))
            _, rad = model.relation(tensor(relation_data["pair_src1"][ids], dev), tensor(relation_data["pair_src1_mask"][ids], dev), tensor(relation_data["pair_pos2"][ids], dev), tensor(relation_data["pair_pos2_mask"][ids], dev))
            _, rcb = model.relation(tensor(relation_data["pair_src2"][ids], dev), tensor(relation_data["pair_src2_mask"][ids], dev), tensor(relation_data["pair_pos1"][ids], dev), tensor(relation_data["pair_pos1_mask"][ids], dev))
            link_error.extend([np.zeros(len(ids), int), np.zeros(len(ids), int), np.ones(len(ids), int), np.ones(len(ids), int)])
            link_score.extend([torch.sigmoid(rab).cpu().numpy(), torch.sigmoid(rcd).cpu().numpy(), torch.sigmoid(rad).cpu().numpy(), torch.sigmoid(rcb).cpu().numpy()])
    link_y = np.concatenate(link_error) if link_error else np.zeros(0, int)
    link_s = np.concatenate(link_score) if link_score else np.zeros(0, float)
    split_y = []; split_s = []
    for d in node_data:
        node_score, boundary_score, valid = score_fixed_nodes(model, d, dev)
        false_split_risk = boundary_risk_scores(model, d["node_x"], d["node_mask"], node_score, node_threshold, dev)
        by = np.asarray(d["boundary_label"], int)
        candidate = (node_score[:, None] >= node_threshold) & (boundary_score >= boundary_threshold) & (valid > 0) & (by >= 0)
        split_y.append((by[candidate] <= 0).astype(int)); split_s.append(false_split_risk[candidate])
    split_y_arr = np.concatenate(split_y) if split_y else np.zeros(0, int)
    split_s_arr = np.concatenate(split_s) if split_s else np.zeros(0, float)
    policies = {}
    for name, limit in [("P1", P1_RISK), ("P2", P2_RISK)]:
        policies[name] = {"link": calibrated_risk_threshold(link_y, link_s, limit),
                          "false_split": calibrated_risk_threshold(split_y_arr, split_s_arr, limit)}
    return {"method": "training-sequence-only monotone score cutoff; largest cutoff with one-sided 95% Clopper-Pearson selected-set UCB <= fixed risk limit",
            "score_direction": "lower is safer", "link_samples": int(len(link_y)),
            "false_split_candidate_samples": int(len(split_y_arr)), "policies": policies}
...
def fit_inner_model(outer: str, validation_seq: str) -> tuple[Path, dict[str, Any]]:
    ...
    for group, seq in enumerate(training):
        d = mot20_fixed_node_arrays(seq, True); d["node_group"] = np.full(len(d["node_label"]), group, np.int16); node_data.append(d)
    node = {name: np.concatenate([d[name] for d in node_data]) for name in ["node_x", "node_mask", "node_label", "boundary_label", "aba_event", "node_group"]}
    rel_data = []
    for group, seq in enumerate(training):
        d = mot20_relation_triplets(seq); d["rel_group"] = np.full(len(d["rel_gap"]), group, np.int16); d["pair_group"] = np.full(len(d["pair_src1"]), group, np.int16); rel_data.append(d)
    relation_names = ["rel_src", "rel_src_mask", "rel_pos", "rel_pos_mask", "rel_out_neg", "rel_out_neg_mask", "rel_in_neg", "rel_in_neg_mask", "rel_gap", "rel_group",
                      "pair_src1", "pair_src1_mask", "pair_pos1", "pair_pos1_mask", "pair_src2", "pair_src2_mask", "pair_pos2", "pair_pos2_mask", "pair_group"]
    rel = {name: np.concatenate([d[name] for d in rel_data]) for name in relation_names}
    train_data = {**node, **rel}
    ...
    for epoch in range(MOT20_FINETUNE_EPOCHS):
        model.train(); parts_epoch = []
        nb = list(sample_indices(len(node["node_label"]), BATCH_NODES, rng)); rb = list(sample_indices(len(rel["rel_group"]), BATCH_RELATIONS, rng))
        pb = list(sample_indices(len(rel["pair_group"]), BATCH_RELATIONS, rng)) if len(rel["pair_group"]) else [np.zeros(0, np.int64)]
        for step in range(max(len(nb), len(rb), len(pb))):
            total, qn, qr, parts = training_objective(model, train_data, nb[step % len(nb)], rb[step % len(rb)], pb[step % len(pb)], dev, qn, qr)
            if not torch.isfinite(total): raise RuntimeError(f"non-finite inner loss outer={outer} valid={validation_seq}")
            optimizer.zero_grad(set_to_none=True); total.backward(); nn.utils.clip_grad_norm_(model.parameters(), 5.0); optimizer.step(); parts_epoch.append(parts)
        history.append({"epoch": epoch + 1, **{f"mean_{k}": float(np.mean([p[k] for p in parts_epoch])) for k in parts_epoch[0]}})
    ...
    risk_calibration = calibrate_model_risk(model, node_data, rel, node_threshold, boundary_threshold, dev)
    root.mkdir(parents=True, exist_ok=True)
    risk_path = root / "risk_calibration.json"; json_write(risk_path, risk_calibration, refuse_existing=True)
    ...
    manifest = {
        ...
        "validation_labels_read_before_freeze": False, "outer_gt_read": False, "outer_teacher_action_read": False,
        "external_checkpoint_sha256": external_manifest["checkpoint_sha256"],
        "model": str(model_path), "model_sha256": sha256_file(model_path),
        "identity_scaler": {"type": "none/fixed raw normalized appearance and predeclared geometry", "sha256": hashlib.sha256(b"M23-59-identity-scaler-v2").hexdigest()},
        "node_threshold": node_threshold, "boundary_threshold": boundary_threshold,
        "risk_calibration": risk_calibration, "risk_calibration_path": str(risk_path),
        "risk_calibration_sha256": sha256_file(risk_path),
...
def evaluate_inner_validation(outer: str, validation_seq: str, model_path: Path, frozen: dict[str, Any]) -> dict[str, Any]:
    ...
    # The model/scaler/threshold/risk manifest already exists before the first validation-label read.
    ...
    report = {
        ...
        "validation_labels_read_after_model_freeze": True,
...
def freeze_outer_p0_manifest(outer: str, representation: dict[str, Any]) -> dict[str, Any]:
    ...
    candidate_manifest = M55_ROOTS[outer] / "frozen_candidate_graph/freeze_manifest.json"
    manifest = {
        ...
        "scaler_sha256": hashlib.sha256(b"M23-59-identity-scaler-v2").hexdigest(),
...
def build_learned_tracker(seq: str, model_path: Path, thresholds: dict[str, float], policy: str,
    ...
    calibration = thresholds["risk_calibration"]["policies"][policy]
    cut_risk_threshold = float(calibration["false_split"]["score_threshold"])
    link_risk_threshold = float(calibration["link"]["score_threshold"])
    dev = device(); model = load_inner_model(model_path, dev)
    ...
    for cid in range(len(node_score)):
        ...
        for pos in range(1, MAX_NODE_ROWS):
            if cut_risk_threshold >= 0.0 and valid[cid, pos - 1] and boundary_score[cid, pos - 1] >= float(thresholds["boundary_threshold"]) and boundary_risk[cid, pos - 1] <= cut_risk_threshold:
    ...
    utility[(link_risk_threshold < 0.0) | (edge_risk > link_risk_threshold)] = -np.inf
    ...
    if teacher_audit:
        ...
        teacher_key = {(int(r.src_chunk), int(r.dst_chunk)): int(r.teacher_same_identity_forward) for r in teacher_edges.itertuples()}
        parent_pairs = set((int(r.src_chunk), int(r.dst_chunk)) for r in edges[edges.parent_edge > 0].itertuples())
        selected_pairs = set((int(r.src_chunk), int(r.dst_chunk)) for r in selected.itertuples())
        cut_parent = parent_pairs - selected_pairs
        selected_cross = selected[selected.parent_edge <= 0]
        wrong_cross = sum(int(teacher_key.get((int(r.src_chunk), int(r.dst_chunk)), 0) <= 0) for r in selected_cross.itertuples())
        false_cut_parent = sum(int(teacher_key.get(pair, 0) > 0) for pair in cut_parent)
        ...
        identity_edits = cut_count + len(cut_parent) + len(selected_cross)
        errors = false_cuts + false_cut_parent + wrong_cross
        audit = {"false_boundary_cuts": false_cuts, "selected_boundary_cuts": cut_count,
                 "false_parent_cuts": false_cut_parent, "cut_parent_edges": len(cut_parent),
                 "wrong_cross_links": wrong_cross, "selected_cross_links": len(selected_cross),
                 "catastrophic_errors": errors, "identity_edits": identity_edits,
                 "catastrophic_risk_ucb": clopper_pearson_upper(errors, identity_edits)}
    report = {
        "experiment": "M23-59 hierarchical learned tracker", "seq": seq, "policy": policy,
        "external_supervision": True, "risk_limit": risk_limit,
        "risk_calibration": calibration, "cut_false_split_score_threshold": cut_risk_threshold,
        "link_risk_score_threshold": link_risk_threshold,
        ...
        "teacher_audit": audit, "identity_edits": int(audit["identity_edits"]) if audit is not None else None,
...
def run_inner_exact_gate(outer: str) -> dict[str, Any]:
    ...
    for policy, folds in policies.items():
        ...
        eligible[policy] = {
            ...
            "identity_edits": int(sum(json.loads((ROOT / "nested_loso" / outer / f"inner_valid_{f['validation_sequence']}" / f"tracker_{policy}/report.json").read_text())["identity_edits"] for f in folds)),
        }
...
def fit_outer_final_model(outer: str) -> dict[str, Any]:
    ...
    nodes = []
    for group, seq in enumerate(training):
        d = mot20_fixed_node_arrays(seq, True); d["node_group"] = np.full(len(d["node_label"]), group, np.int16); nodes.append(d)
    node = {name: np.concatenate([d[name] for d in nodes]) for name in ["node_x", "node_mask", "node_label", "boundary_label", "node_group"]}
    relations = []
    for group, seq in enumerate(training):
        d = mot20_relation_triplets(seq); d["rel_group"] = np.full(len(d["rel_gap"]), group, np.int16); d["pair_group"] = np.full(len(d["pair_src1"]), group, np.int16); relations.append(d)
    relation_names = ["rel_src", "rel_src_mask", "rel_pos", "rel_pos_mask", "rel_out_neg", "rel_out_neg_mask", "rel_in_neg", "rel_in_neg_mask", "rel_group",
                      "pair_src1", "pair_src1_mask", "pair_pos1", "pair_pos1_mask", "pair_src2", "pair_src2_mask", "pair_pos2", "pair_pos2_mask", "pair_group"]
    rel = {name: np.concatenate([d[name] for d in relations]) for name in relation_names}; train_data = {**node, **rel}
    ...
    for epoch in range(MOT20_FINETUNE_EPOCHS):
        ...
        for step in range(max(len(nb), len(rb), len(pb))):
            total, qn, qr, parts = training_objective(model, train_data, nb[step % len(nb)], rb[step % len(rb)], pb[step % len(pb)], dev, qn, qr)
            if not torch.isfinite(total): raise RuntimeError(f"non-finite final outer loss outer={outer}")
            optimizer.zero_grad(set_to_none=True); total.backward(); nn.utils.clip_grad_norm_(model.parameters(), 5.0); optimizer.step(); parts_epoch.append(parts)
        history.append({"epoch": epoch + 1, **{f"mean_{k}": float(np.mean([p[k] for p in parts_epoch])) for k in parts_epoch[0]}})
    ...
    risk_calibration = calibrate_model_risk(model, nodes, rel, node_threshold, boundary_threshold, dev)
    root.mkdir(parents=True, exist_ok=True)
    risk_path = root / "risk_calibration.json"; json_write(risk_path, risk_calibration, refuse_existing=True)
    model_path = root / "model.pt"; history_path = root / "history.json"
...
def freeze_outer_policy(outer: str) -> dict[str, Any]:
    ...
    if exact["selected_policy"] == "P0":
        ...
        manifest = {
            ...
            "scaler_sha256": hashlib.sha256(b"M23-59-identity-scaler-v2").hexdigest(), "risk_calibration_sha256": None,
            "boundary_policy": "no-op", "flow_policy": "byte-exact M23-46",
            "candidate_graph_sha256": sha256_file(M55_ROOTS[outer] / "frozen_candidate_graph/freeze_manifest.json"),
            "outer_gt_read": False, "outer_teacher_action_read": False,
            "tracker": {"path": str(tracker), "sha256": sha256_file(tracker), "byte_exact_m23_46": True},
            "representation_gate_sha256": sha256_file(ROOT / "nested_loso" / outer / "representation_gate.json"),
    ...
    out = ROOT / "outer_policies" / outer; path = out / "outer_policy_manifest.json"
    if path.exists(): return json.loads(path.read_text())
    tracker_root = ROOT / "outer_tracker_freeze" / outer
    tracker_report = build_learned_tracker(outer, Path(model["model"]), model, exact["selected_policy"], tracker_root, teacher_audit=False)
    manifest = {
        "experiment": "M23-59 strict outer policy", "outer": outer,
        ...
        "boundary_policy": {"node_threshold": model["node_threshold"], "boundary_threshold": model["boundary_threshold"], "multiple_cuts": True,
                            "risk_calibration": model["risk_calibration"]["policies"][exact["selected_policy"]]},
        "flow_policy": {"candidate_generator": "fixed M23-55 K256/K32", "utility": "relation logit +0.25 parent -0.25 cross; calibrated risk-filtered", "one_to_one": True},
        "candidate_graph_sha256": tracker_report["graph"]["edges_sha256"],
        "selected_flow_sha256": tracker_report["graph"]["selected_sha256"],
        "tracker": tracker_report["tracker"], "tracker_report_sha256": sha256_file(tracker_root / "report.json"),
        "outer_tracker_frozen_before_outer_gt": True,
        "outer_gt_read": False, "outer_teacher_action_read": False,
        "representation_gate_sha256": sha256_file(ROOT / "nested_loso" / outer / "representation_gate.json"),
        "inner_exact_gate_sha256": sha256_file(ROOT / "nested_loso" / outer / "inner_exact_gate.json"),
        "inner_trackeval_runs": int(exact["trackeval_runs"]),
...
def evaluate_outer_policies() -> dict[str, Any]:
    ...
    for seq in MOT20_SEQS:
        ...
        frozen_tracker = Path(manifest["tracker"]["path"])
        if sha256_file(frozen_tracker) != manifest["tracker"]["sha256"]:
            raise RuntimeError(f"{seq}: frozen outer tracker SHA mismatch")
        shutil.copyfile(frozen_tracker, results_dir / f"{seq}.txt")
        # Once at most per held fold, after all policy manifests and trackers froze.
