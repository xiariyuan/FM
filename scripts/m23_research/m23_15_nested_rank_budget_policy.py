...
# Research artifact for the MOT20 M23 nested rank-budget audit.
...
OUT = Path("outputs/mot20_m23_20260718/m23_15_nested_rank_budget_policy_v1")
NAME = "nested_rank_budget_ood_policy_v1"
SCORE_COLUMN = "pred_expected_transaction_utility"
...
def topk_disjoint(frame: pd.DataFrame, top_k: int) -> pd.DataFrame:
    candidates = frame.copy()
    candidates["policy_score"] = candidates[SCORE_COLUMN].to_numpy(float)
...
def load_inner_predictions(outer_held: str) -> dict[str, pd.DataFrame]:
    ...
    return {
        inner_held: pd.read_parquet(root / f"{inner_held}_predictions.parquet")
        for inner_held in SEQS
        if inner_held != outer_held
    }


def calibrate_rank_budget(
    ...
    for top_k in TOP_K_GRID:
        ...
        for seq, prediction in inner_predictions.items():
            selected = topk_disjoint(prediction, top_k)
    ...
    if positive:
        best = max(
            positive,
            key=lambda item: (item["median_true_delta_sum"], -item["top_k"]),
        )
        chosen = int(best["top_k"])
    return chosen, {
        "rule": (
            "choose the pre-registered disjoint rank budget with the largest "
            "positive median true transaction-utility sum over the three inner "
            "held sequences; ties choose the smaller budget; otherwise no-op"
        ),
        "rank_score": SCORE_COLUMN,
...
def main() -> None:
    ...
    for outer_held in SEQS:
        ...
        calibrated_top_k, calibration = calibrate_rank_budget(inner_predictions)
        outer = pd.read_parquet(
            OUTER_PRED / f"{outer_held}_chain_expected_utility_predictions.parquet"
        )

        inner_q90 = {seq: probability_q90(frame) for seq, frame in inner_predictions.items()}
        ...
        selected = (
            ...
            else topk_disjoint(outer, applied_top_k)
        )
        ...
        fold_reports.append(
            {
                ...
                "ood_gate": {
                    "rule": (
                        ...
                        "q90 exceeds the maximum q90 of the three inner "
                        "held-sequence predictions"
                    ),
    ...
    frozen_protocol = {
        "status": "outer rank budget and transaction selection frozen before TrackEval",
        "selection_sha256": selection_sha256,
        "policy_sha256": policy_sha256,
        "outer_gt_used_in_model_or_policy_selection": False,
        "inner_gt_use": (
            "training-sequence chain-transaction utility labels only for median "
            "rank-budget calibration"
        ),
        "feature_gt_use": "none; rank score and q90 OOD gate use GT-free predictions",
        "top_k_grid_provenance": (
            "subset of the pre-existing M23-12 v2 diagnostic ranking grid; fixed "
            "before M23-15 TrackEval"
        ),
        "folds": fold_reports,
    }
    ...
    report = {
        "name": NAME,
        "status": (
            "nested sequence-LOSO rank-budget audit on reused development "
