...
"""M23-35: strict nested-LOSO catastrophic-risk-aware component matcher.
...
def evaluate_single(
    ...
    return evaluate_combined(track_results, output_root, tracker_name, (sequence,))


def policy_sort_key(row: Mapping[str, object]) -> Tuple[float, float, int, int]:
    return (
        float(row["HOTA"]),
        float(row["AssA"]),
        -int(row["IDSW"]),
...
def main() -> None:
    ...
    protocol = {
        "experiment": "M23-35 catastrophic-risk-aware conflict-component matcher",
    ...
    m25 = load_module(
        "m23_35_m25", "scripts/m23_research/m23_25_sequence_calibrated_transaction_graph.py"
    )
    base = load_module(
        "m23_35_base", "scripts/m23_research/m23_12_train_chain_transaction_loso.py"
    )
    chain = load_module(
        "m23_35_chain", "scripts/m23_research/m23_12_chain_transaction_oracle.py"
    )
    evaluator = load_module(
        "m23_35_evaluator", "scripts/m23_research/m23_11_eval_utility_graph.py"
    )
    ...
    for candidate_index, policy in enumerate(policies):
        ...
        metrics = evaluate_combined(
            track_root,
            candidate_root,
            f"m23_35_inner_{candidate_index}",
            training_sequences,
        )
        row = {
