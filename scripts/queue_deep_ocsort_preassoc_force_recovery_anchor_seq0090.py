...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Queue anchored recovery-memory seq0090 experiments after the current fragment-memory queue finishes."
    )
    ...
    parser.add_argument(
        ...
        default=str(
            REPO_ROOT
            / "outputs"
            / "deep_ocsort_preassoc_force_recovery_memory_seq0090_debug"
            / "summary.csv"
        ),
    )
    parser.add_argument(
...
def main() -> int:
    ...
    run_root = (
        ...
        if args.out_root
        else (REPO_ROOT / "outputs" / f"deep_ocsort_preassoc_force_recovery_anchor_seq0090_{timestamp_tag()}").resolve()
    )
    ...
    variants = [
        {
            "step": "anchor_gap3_safe015",
            "name": "anchor_gap3_safe015",
            "notes": "require a real anchor before temporal carry, keep a three-frame fragment window, and use a conservative 0.15 edge-deficit safety band during extension",
        ...
        {
            "step": "anchor_gap2_safe015",
            "name": "anchor_gap2_safe015",
            "notes": "start from the current best short-fragment setting, then add anchor gating so only already-proven recovery fragments are allowed to extend",
            "seq_names": [args.seq_name],
            "force_rewrite_min_score": 0.65,
            "force_rewrite_min_box_iou": 0.55,
            "force_rewrite_min_neighborhood_gain": 0.0,
        ...
        {
            "step": "anchor_gap3_safe020",
            "name": "anchor_gap3_safe020",
            "notes": "keep the anchored three-frame fragment model but slightly widen the extension safety band to test whether the anchor is right and only the extension margin is too strict",
            "seq_names": [args.seq_name],
            "force_rewrite_min_score": 0.65,
            "force_rewrite_min_box_iou": 0.55,
            "force_rewrite_min_neighborhood_gain": 0.0,
            ...
            "recovery_memory_anchor_min_raw_neighborhood_gain": 0.05,
            "recovery_memory_anchor_max_edge_deficit": 0.35,
            "recovery_memory_extension_max_edge_deficit_delta": 0.20,
        },
    ]
    ...
    queue_rows: List[Dict[str, object]] = [
        {
            ...
            "started_at": "",
            "finished_at": "",
            "notes": "wait for the current seq0090 fragment-memory queue to finish before launching anchored recovery-fragment experiments",
        }
    ]
    ...
    overall_notes = "completed anchored recovery-memory follow-up queue"
    ...
    append_registry(
        ...
        run_root,
        "running",
        "waiting for current seq0090 fragment-memory queue to finish",
        args.registry_csv,
    )
