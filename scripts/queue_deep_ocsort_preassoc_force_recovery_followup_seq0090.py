...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Follow-up queue that only relaxes trapped-owner recovery thresholds around the recovery_bias_v1 sweet spot."
    )
...
def main() -> int:
    ...
    run_root = (
        ...
        if args.out_root
        else (REPO_ROOT / "outputs" / f"deep_ocsort_preassoc_force_recovery_followup_seq0090_{timestamp_tag()}").resolve()
    )
    reuse_raw_from = Path(args.reuse_raw_from).expanduser().resolve()

    variants = [
        {
            "step": "recovery_followup_m001",
            "name": "recovery_followup_m001",
            "notes": "keep the proven recovery-biased setting but allow a tiny negative gain only for trapped-owner recovery rows",
            "seq_names": [args.seq_name],
            "force_rewrite_min_score": 0.65,
            "force_rewrite_min_box_iou": 0.55,
            "force_rewrite_min_neighborhood_gain": 0.0,
            "trapped_owner_min_neighborhood_gain": -0.01,
            "reroute_ready_min_neighborhood_gain": 0.0,
            "trapped_owner_negative_gain_min_challenger_alt_box_iou": 0.15,
        ...
        {
            "step": "recovery_followup_m002_r005",
            "name": "recovery_followup_m002_r005",
            "notes": "relax trapped-owner recovery slightly more while tightening reroute-ready rows back to a positive gain floor",
            "seq_names": [args.seq_name],
            "force_rewrite_min_score": 0.65,
            "force_rewrite_min_box_iou": 0.55,
            "force_rewrite_min_neighborhood_gain": 0.0,
            "trapped_owner_min_neighborhood_gain": -0.02,
            "reroute_ready_min_neighborhood_gain": 0.05,
            "trapped_owner_negative_gain_min_challenger_alt_box_iou": 0.12,
            "force_rewrite_max_age_gap": 200,
            "force_rewrite_max_owner_alt_det_box_iou": 0.50,
            "reroute_ready_min_box_iou": 0.80,
            "keep_challenger_alt_weight": 0.25,
    ...
    overall_notes = "completed recovery follow-up queue"
    ...
    for index, variant in enumerate(variants):
        ...
        if index == 0:
            append_registry(
                ...
                run_root,
                "running",
                "started recovery follow-up queue",
                args.registry_csv,
            )
