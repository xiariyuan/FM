...
RUN_ROOT="${RUN_ROOT:?Please set RUN_ROOT}"
TS="${TS:?Please set TS}"
RUN_ROOT="$(realpath -m "${RUN_ROOT}")"
LOG_PATH="${RUN_ROOT}/resume.log"
...
echo "[resume-start] $(date '+%F %T %z')"
echo "[run_root] ${RUN_ROOT}"

if [[ $(find "${BASE_RESULTS}" -maxdepth 1 -name '*.txt' | wc -l) -lt 7 ]]; then
  echo "Base MOT17 results are incomplete under ${BASE_RESULTS}" >&2
  exit 1
fi
...
echo "[resume-step] clean partial heuristic/learned artifacts"
rm -rf "${BOT_ROOT}/YOLOX_outputs/${HEUR_EXP}" \
       "${BOT_ROOT}/YOLOX_outputs/${LEARNED_EXP}" \
       "${BOT_ROOT}/YOLOX_outputs/${MOT20_LEARNED_EXP}" \
       "${HEUR_LOG_DIR}" \
       "${LEARNED_LOG_DIR}" \
       "${MOT20_LEARNED_LOG_DIR}" \
       "${RUN_ROOT}/eval/mot17_heuristic" \
       "${RUN_ROOT}/eval/mot17_learned" \
       "${RUN_ROOT}/eval/mot20_learned"

echo "[resume-step] MOT17 heuristic+fresh logs"
...
echo "[header] $(head -n 1 "${HEUR_COMBINED}")"
...
echo "[resume-step] summarize heuristic logs"
...
echo "[resume-step] train alpha/r calibrator"
...
echo "[resume-step] MOT17 learned+logs"
...
echo "[resume-step] summarize learned logs"
"${PYTHON_BIN}" "${REPO_ROOT}/scripts/summarize_laplace_pair_logs.py" \
  "${LEARNED_COMBINED}" \
  --out-dir "${LEARNED_LOG_DIR}/_combined"

echo "[resume-step] evaluate MOT17 base/heuristic/learned"
...
echo "[resume-step] MOT20 learned follow-up"
