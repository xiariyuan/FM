...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Recovery-biased stale rewrite queue for seq0090 after neighborhood-gain diagnosis."
    )
...
def main() -> int:
    ...
    run_root = (
        ...
        if args.out_root
        else (REPO_ROOT / "outputs" / f"deep_ocsort_preassoc_force_recovery_seq0090_{timestamp_tag()}").resolve()
    )
    reuse_raw_from = Path(args.reuse_raw_from).expanduser().resolve()

    variants = [
        {
            "step": "recovery_bias_v1",
            "name": "recovery_bias_v1",
            "notes": "reward trapped-owner recovery cases and down-rank reroute-ready duplicate cleanups",
        ...
        {
            "step": "recovery_bias_v2",
            "name": "recovery_bias_v2",
            "notes": "push harder toward trapped-owner recovery and require even cleaner reroute-ready overlaps",
            "seq_names": [args.seq_name],
            "force_rewrite_min_score": 0.65,
            "force_rewrite_min_box_iou": 0.55,
            "force_rewrite_min_neighborhood_gain": 0.0,
            "force_rewrite_max_age_gap": 200,
            "force_rewrite_max_owner_alt_det_box_iou": 0.50,
            "reroute_ready_min_box_iou": 0.82,
            "keep_challenger_alt_weight": 0.20,
            "rewrite_owner_alt_weight": 1.00,
            "shared_alt_penalty": 0.25,
            "trapped_owner_bonus": 0.90,
            "reroute_ready_penalty": 0.70,
        },
    ]
    ...
    overall_notes = "completed recovery-biased rewrite queue"
    ...
    for index, variant in enumerate(variants):
        ...
        if index == 0:
            append_registry(
                ...
                run_root,
                "running",
                "started recovery-biased rewrite queue",
                args.registry_csv,
            )
