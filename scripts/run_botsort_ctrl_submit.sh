...
MODE="${1:?submit mode is required: base|heuristic}"
DATASET="${2:-MOT17}"
OUT_DIR="${3:-${REPO_ROOT}/outputs/${MODE}_${DATASET,,}_submit_$(date +%Y%m%d_%H%M%S)}"
DATA_ROOT="${4:-/gemini/code/datasets}"

if [[ "${MODE}" != "base" && "${MODE}" != "heuristic" ]]; then
  echo "Unsupported MODE=${MODE}; expected base|heuristic" >&2
  exit 2
fi
...
CTRL_LABEL="${CTRL_LABEL:-${MODE}}"
MOT17_PRIVATE_DET="${MOT17_PRIVATE_DET:-0}"
MOT17_PRIMARY_EXT="${MOT17_PRIMARY_EXT:-FRCNN}"

if [[ "${MOT17_PRIVATE_DET}" == "1" ]]; then
  DEFAULT_PROFILE_BASE="mot17_private_ctrl_base"
  DEFAULT_PROFILE_HEURISTIC="mot17_private_ctrl_heuristic"
else
  DEFAULT_PROFILE_BASE="mot17_public_ctrl_base"
  DEFAULT_PROFILE_HEURISTIC="mot17_public_ctrl_heuristic"
fi
if [[ "${MODE}" == "base" ]]; then
  TRACK_PROFILE="${TRACK_PROFILE:-${DEFAULT_PROFILE_BASE}}"
else
  TRACK_PROFILE="${TRACK_PROFILE:-${DEFAULT_PROFILE_HEURISTIC}}"
fi

ZIP_NAME="mot17_${CTRL_LABEL}_test_submission_$(date +%Y%m%d_%H%M%S).zip"
EXPERIMENT_NAME="ctrl_submit_mot17_${CTRL_LABEL}_$(date +%Y%m%d_%H%M%S)"
...
(
  cd "${BS_ROOT}"
  cmd=("${TRACK_ARGS[@]}")
  cmd+=(--run-manifest-path "${RUN_MANIFEST}")
...
if [[ -f "${RUN_MANIFEST}" ]]; then
  "${PYTHON_BIN}" "${MANIFEST_TOOL}" \
    ...
    --set "submit.mode=\"${MODE}\"" \
    --set "submit.ctrl_label=\"${CTRL_LABEL}\"" \
