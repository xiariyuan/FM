...
OUT_DIR="${1:-${REPO_ROOT}/outputs/local_conflict_graph_learned_commit_$(date +%Y%m%d_%H%M%S)}"
RUN_NAME="${2:-local_conflict_graph_learned_commit}"
GRAPH_CKPT="${3:-}"
TOPK="${4:-8}"
MIN_DETECTIONS="${5:-2}"
...
MAX_DETECTIONS="${7:-8}"
MAX_TRACKS="${8:-32}"
DETECTOR_FILTER_CSV="${9:-}"
VAL_SEQUENCES_CSV="${10:-}"
NOTES="${11:-learned conservative local-conflict commit evaluation}"
...
"${PYTHON_BIN}" - "${RESULT_CSV}" "${SUMMARY_CSV}" "${RUN_NAME}" "${OUT_DIR}" "${BASE_CONFIG}" "${BASE_CKPT}" "${HOST_VARIANT}" "${GRAPH_CKPT}" "${TOPK}" "${MIN_DETECTIONS}" "${MIN_COMMITTED_MATCHES}" "${MAX_DETECTIONS}" "${MAX_TRACKS}" "${DETECTOR_FILTER_CSV}" "${VAL_SEQUENCES_CSV}" <<'PY'
import csv
import sys
...
detector_filter_csv = sys.argv[14]
val_sequences_csv = sys.argv[15]
...
"${PYTHON_BIN}" - "${PROFILE_PATH}" "${BASE_CONFIG}" "${BASE_CKPT}" "${HOST_VARIANT}" "${RUN_NAME}" "${GRAPH_CKPT}" "${TOPK}" "${MIN_DETECTIONS}" "${MIN_COMMITTED_MATCHES}" "${MAX_DETECTIONS}" "${MAX_TRACKS}" "${DETECTOR_FILTER_CSV}" "${VAL_SEQUENCES_CSV}" "${NOTES}" <<'PY'
import json
import sys
from pathlib import Path
...
detector_filter_csv = str(sys.argv[12] or "").strip()
val_sequences_csv = str(sys.argv[13] or "").strip()
notes = str(sys.argv[14] or "").strip()
...
"${PYTHON_BIN}" - "${RESULT_CSV}" "${SUMMARY_CSV}" "${MANIFEST_PATH}" "${BASE_CONFIG}" "${BASE_CKPT}" "${HOST_VARIANT}" "${RUN_NAME}" "${OUT_DIR}" "${GRAPH_CKPT}" "${TOPK}" "${MIN_DETECTIONS}" "${MIN_COMMITTED_MATCHES}" "${MAX_DETECTIONS}" "${MAX_TRACKS}" "${DETECTOR_FILTER_CSV}" "${VAL_SEQUENCES_CSV}" "${STATUS}" <<'PY'
import csv
import json
import sys
from pathlib import Path
...
detector_filter_csv = sys.argv[15]
val_sequences_csv = sys.argv[16]
status = sys.argv[17]
...
if manifest_path.is_file():
    ...
    for key in ("eligible_clusters", "replaced_clusters", "matched_dets", "deferred_dets", "blocked_tracks", "skipped_large_clusters"):
        if key in diag:
...
"${PYTHON_BIN}" "${REPO_ROOT}/scripts/append_experiment_record.py" \
  ...
  --script "scripts/run_local_conflict_graph_learned_commit_generic.sh" \
  --dataset "MOT17" \
  --split "${SPLIT_LABEL}" \
  ...
  --log-path "${LOG_PATH}" \
  --notes "${NOTES}" \
  --extra "host_variant=${HOST_VARIANT}" "host_config_path=${BASE_CONFIG}" "host_checkpoint=${BASE_CKPT}" "graph_mode=learned_commit" "graph_topk=${TOPK}" "graph_min_detections=${MIN_DETECTIONS}" "graph_min_committed_matches=${MIN_COMMITTED_MATCHES}" "graph_max_detections=${MAX_DETECTIONS}" "graph_max_tracks=${MAX_TRACKS}" "detector_filter=${DETECTOR_FILTER_CSV}" "val_sequences=${VAL_SEQUENCES_CSV}"
...
if ! "${PYTHON_BIN}" "${REPO_ROOT}/scripts/post_experiment_pro_bundle.py" \
  ...
  echo "[local-conflict-graph-learned-commit-generic] warning: failed to build Pro review bundle for ${OUT_DIR}" >&2
fi

echo "[local-conflict-graph-learned-commit-generic] status=${STATUS} out_dir=${OUT_DIR} host=${HOST_VARIANT}"
