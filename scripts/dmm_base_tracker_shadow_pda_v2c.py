#!/usr/bin/env python3
"""ShadowPDA-V2c: hypothesis-competition posterior with appearance-overtrust risk.

V2c keeps the V2b online lost-vs-active-vs-clutter posterior and adds an
appearance-overtrust clutter term: low detection validity should not be fully
compensated by high lost-identity appearance similarity.
"""
...
class ShadowPDAV2cTracker(spda.ShadowPDATracker):
    def __init__(self, cfg, osr, shadow, v2: V2cConfig):
        super().__init__(cfg, osr, shadow)
        self.v2 = v2
        self.shadow_stats.update({
            'v2c_public_candidates': 0,
            'v2c_public_recovered': 0,
            'v2c_public_blocked': 0,
        })
    ...
    def _public_score(
        ...
        appearance_overtrust = clip01(float(lost_app)) * det_weakness
        ...
        z = {
            ...
            'z_appearance_overtrust': float(appearance_overtrust),
        ...
        score = (
            ...
            - float(self.v2.w_clutter) * z['z_clutter']
            - float(self.v2.w_entropy) * z['z_entropy']
            - float(self.v2.w_det_weak) * z['z_det_weakness']
            - float(self.v2.w_app_overtrust) * z['z_appearance_overtrust']
            - float(self.v2.w_temporal_weak) * z['z_temporal_weakness']
            - float(self.v2.w_age) * z['z_age']
        )
        z['lost_likelihood'] = float(lost_likelihood)
    ...
    def _try_ultra_low_recover(self, lost_tracks: Sequence[spda.base.DMMTrack], low_dets: Sequence[spda.base.DMMTrack], active_tracks: Sequence[spda.base.DMMTrack] | None = None) -> List[Tuple[int, spda.base.DMMTrack]]:
        ...
        for idx, track in enumerate(lost_tracks):
            ...
            self.shadow_stats['v2c_public_candidates'] += 1
            ...
            public_score, z = self._public_score(st, det, lost_age, lost_app, identity_margin, active_best_sim, active_overlap_now)
            ...
            ok = bool(public_score >= float(self.v2.theta_public) and all(bool(v) for v in safety.values()))
            ...
            if ok:
                ...
                self.shadow_stats['v2c_public_recovered'] += 1
            else:
                self.shadow_stats['ultra_low_blocked'] += 1
                self.shadow_stats['v2c_public_blocked'] += 1
        return out


def parse_args() -> tuple[argparse.Namespace, V2cConfig]:
    ...
    v2_parser.add_argument('--v2-w-app-overtrust', type=float, default=0.7)
    ...
    v2 = V2cConfig(
        ...
        w_app_overtrust=float(v2_args.v2_w_app_overtrust),
...
def make_tracker(args: argparse.Namespace, v2: V2cConfig) -> ShadowPDAV2cTracker:
    ...
    return ShadowPDAV2cTracker(cfg, osr, shadow, v2)
