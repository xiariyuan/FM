...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Recorded local-neighborhood queue around the current best Deep-OC-SORT pre-association stale-competition configuration."
    )
    parser.add_argument("--out-root", default="")
    parser.add_argument(
        "--reference-run-root",
        default=str(REPO_ROOT / "outputs" / "deep_ocsort_preassoc_competition_followup_best_confirm_20260405_1"),
    )
    parser.add_argument(
        "--reuse-raw-from",
        default=str(REPO_ROOT / "outputs" / "deep_ocsort_preassoc_competition_followup_best_confirm_20260405_1"),
...
def append_registry(summary_csv: Path, run_root: Path, status: str, notes: str, registry_csv: str) -> None:
    cmd = [
        ...
        status,
        "--script",
        "scripts/queue_deep_ocsort_preassoc_competition_local_neighborhood.py",
        "--dataset",
        "MOT17",
        "--split",
        "val_half",
        ...
        run_root.name,
        "--tag",
        "deep_ocsort_preassoc_competition_local_neighborhood",
        "--run-root",
        str(run_root),
        "--summary-csv",
        str(summary_csv),
...
def build_variants() -> List[Dict[str, object]]:
    ...
    for owner_max_hits, max_owner_edge_deficit in raw_variants:
        odef_tag = f"{int(round(max_owner_edge_deficit * 100)):03d}"
        variants.append(
            {
                "step": f"oh{owner_max_hits}_odef{odef_tag}",
                "variant": (
                    f"owner_max_hits={owner_max_hits},"
                    f"max_owner_edge_deficit={max_owner_edge_deficit:.2f},"
                    "block_owner_on_reclaim=1"
                ),
                "suffix": f"oh{owner_max_hits}_odef{odef_tag}",
                "owner_max_hits": owner_max_hits,
                "max_owner_edge_deficit": max_owner_edge_deficit,
                "notes": "local neighborhood around confirmed best oh8_odef020",
            }
        )
    return variants
...
def main() -> None:
    ...
    queue_name = (
        ...
        if args.out_root
        else f"deep_ocsort_preassoc_competition_local_neighborhood_{timestamp_tag()}"
    )
    ...
    upsert_decision_row(
        ...
        {
            "step": "reference",
            "variant": "owner_max_hits=8,max_owner_edge_deficit=0.20,block_owner_on_reclaim=1",
            "status": "success",
            ...
            "notes": "confirmed best before local neighborhood extend",
        },
    )
    ...
    append_registry(
        ...
        out_root,
        "running",
        "preassoc competition local neighborhood queue started" if not existing_queue_rows else "preassoc competition local neighborhood queue resumed",
        registry_csv,
    )
    ...
    try:
        ...
        append_registry(
            ...
            out_root,
            "success",
            f"preassoc competition local neighborhood queue finished best_step={best_step} best_delta_HOTA={best_hota:.3f}",
            registry_csv,
        )
        ...
        write_rows(summary_csv, QUEUE_FIELDS, queue_rows)
        write_rows(decision_csv, DECISION_FIELDS, decision_rows)
        append_registry(summary_csv, out_root, "failed", f"preassoc competition local neighborhood queue failed: {exc}", registry_csv)
        raise
