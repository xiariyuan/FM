...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Write a manifest CSV for local-conflict commit dataset sources.")
    ...
    parser.add_argument("--dataset-tag", default="local_conflict_commit_large_base")
...
def main() -> int:
    ...
    dedupe = {
        (
            ...
            str(existing.get("cases_csv", "")),
            str(existing.get("source_tag", "")),
        )
        for existing in rows
    }
    key = (row["group_jsonl"], row["cases_csv"], row["source_tag"])
    ...
    summary = {
        "out_csv": str(out_csv),
        "rows": len(rows),
        "host_variants": sorted({str(existing.get("host_variant", "")) for existing in rows if str(existing.get("host_variant", "")).strip()}),
        "source_tags": sorted({str(existing.get("source_tag", "")) for existing in rows if str(existing.get("source_tag", "")).strip()}),
        "dataset_tags": sorted({str(existing.get("dataset_tag", "")) for existing in rows if str(existing.get("dataset_tag", "")).strip()}),
        "feature_versions": sorted({str(existing.get("feature_version", "")) for existing in rows if str(existing.get("feature_version", "")).strip()}),
        "status": "ok",
    }
