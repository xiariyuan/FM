#!/usr/bin/env python3
"""ShadowPDA-V2b: hypothesis-competition recovery posterior.

V2b keeps the V2 online shadow-state framework, but changes the public recovery
posterior from positive evidence summation to lost-identity likelihood minus
active-identity and clutter risks.
"""
...
class ShadowPDAV2bTracker(spda.ShadowPDATracker):
    def __init__(self, cfg, osr, shadow, v2: V2bConfig):
        super().__init__(cfg, osr, shadow)
        self.v2 = v2
        self.shadow_stats.update({
            'v2b_public_candidates': 0,
            'v2b_public_recovered': 0,
            'v2b_public_blocked': 0,
        })
    ...
    def _try_ultra_low_recover(self, lost_tracks: Sequence[spda.base.DMMTrack], low_dets: Sequence[spda.base.DMMTrack], active_tracks: Sequence[spda.base.DMMTrack] | None = None) -> List[Tuple[int, spda.base.DMMTrack]]:
        ...
        for idx, track in enumerate(lost_tracks):
            ...
            self.shadow_stats['v2b_public_candidates'] += 1
            ...
            if ok:
                ...
                self.shadow_stats['v2b_public_recovered'] += 1
            else:
                self.shadow_stats['ultra_low_blocked'] += 1
                self.shadow_stats['v2b_public_blocked'] += 1
        return out


def parse_args() -> tuple[argparse.Namespace, V2bConfig]:
...
def make_tracker(args: argparse.Namespace, v2: V2bConfig) -> ShadowPDAV2bTracker:
    ...
    return ShadowPDAV2bTracker(cfg, osr, shadow, v2)
