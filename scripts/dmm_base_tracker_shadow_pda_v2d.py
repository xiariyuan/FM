#!/usr/bin/env python3
"""ShadowPDA-V2d: localization-aware hypothesis-competition posterior.

V2d keeps the V2b lost-vs-active-vs-clutter competition, and adds an online
localization consistency term so low-confidence evidence must be spatially
reliable before it can become public output.
"""
...
class ShadowPDAV2dTracker(spda.ShadowPDATracker):
    def __init__(self, cfg, osr, shadow, v2: V2dConfig):
        super().__init__(cfg, osr, shadow)
        self.v2 = v2
        self.shadow_stats.update({
            'v2d_public_candidates': 0,
            'v2d_public_recovered': 0,
            'v2d_public_blocked': 0,
        })
    ...
    def _public_score(
        ...
        lost_likelihood = (
            ...
            + float(self.v2.w_loc) * z['z_localization']
        )
        score = (
            lost_likelihood
            - float(self.v2.w_active) * z['z_active_risk']
            - float(self.v2.w_loc_risk) * z['z_loc_risk']
    ...
    def _try_ultra_low_recover(self, lost_tracks: Sequence[spda.base.DMMTrack], low_dets: Sequence[spda.base.DMMTrack], active_tracks: Sequence[spda.base.DMMTrack] | None = None) -> List[Tuple[int, spda.base.DMMTrack]]:
        ...
        for idx, track in enumerate(lost_tracks):
            ...
            self.shadow_stats['v2d_public_candidates'] += 1
            ...
            if ok:
                ...
                self.shadow_stats['v2d_public_recovered'] += 1
            else:
                self.shadow_stats['ultra_low_blocked'] += 1
                self.shadow_stats['v2d_public_blocked'] += 1
        return out


def parse_args() -> tuple[argparse.Namespace, V2dConfig]:
...
def make_tracker(args: argparse.Namespace, v2: V2dConfig) -> ShadowPDAV2dTracker:
    ...
    return ShadowPDAV2dTracker(cfg, osr, shadow, v2)
