#!/usr/bin/env python3
"""ShadowPDA-V3-Precision: selective public release.

Keeps V3 decoupled state/output posteriors, but separates ordinary output
validity from high-confidence public release. Low-score evidence may keep a
shadow state alive, while public recovery requires a stricter release posterior.
"""
...
@dataclass
class V3PrecisionConfig:
    theta_public: float = 1.85
    theta_state: float = 2.05
    theta_output: float = 1.25
    theta_output_high: float = 1.45
    theta_state_margin_release: float = 0.10
...
class ShadowPDAV3PrecisionTracker(spda.ShadowPDATracker):
    def __init__(self, cfg, osr, shadow, v2: V3PrecisionConfig):
        super().__init__(cfg, osr, shadow)
        self.v2 = v2
        self.shadow_stats.update({
            'v3_precision_public_candidates': 0,
            'v3_precision_public_recovered': 0,
            'v3_precision_public_blocked': 0,
        })
    ...
    def _public_score(
        ...
        state_margin = float(state_score - float(self.v2.theta_state))
        output_margin = float(output_score - float(self.v2.theta_output))
        release_state_margin = float(state_score - (float(self.v2.theta_state) + float(self.v2.theta_state_margin_release)))
        release_output_margin = float(output_score - float(self.v2.theta_output_high))
        release_margin = min(release_state_margin, release_output_margin)
        score = release_margin + float(self.v2.theta_public)
        z['state_score'] = float(state_score)
        z['output_score'] = float(output_score)
        z['state_margin'] = float(state_margin)
        z['output_margin'] = float(output_margin)
        z['release_state_margin'] = float(release_state_margin)
        z['release_output_margin'] = float(release_output_margin)
        z['release_margin'] = float(release_margin)
    ...
    def _try_ultra_low_recover(self, lost_tracks: Sequence[spda.base.DMMTrack], low_dets: Sequence[spda.base.DMMTrack], active_tracks: Sequence[spda.base.DMMTrack] | None = None) -> List[Tuple[int, spda.base.DMMTrack]]:
        ...
        for idx, track in enumerate(lost_tracks):
            ...
            self.shadow_stats['v3_precision_public_candidates'] += 1
            ...
            ok = bool(public_score >= float(self.v2.theta_public) and z.get('state_score', 0.0) >= float(self.v2.theta_state) and z.get('state_margin', 0.0) >= float(self.v2.theta_state_margin_release) and z.get('output_score', 0.0) >= float(self.v2.theta_output_high) and all(bool(v) for v in safety.values()))
            ...
            if z.get('state_margin', 0.0) < float(self.v2.theta_state_margin_release):
                blocked.append('state_margin_release')
            if z.get('output_score', 0.0) < float(self.v2.theta_output_high):
                blocked.append('output_score_high')
            ...
            if ok:
                ...
                self.shadow_stats['v3_precision_public_recovered'] += 1
            else:
                self.shadow_stats['ultra_low_blocked'] += 1
                self.shadow_stats['v3_precision_public_blocked'] += 1
        return out


def parse_args() -> tuple[argparse.Namespace, V3PrecisionConfig]:
    ...
    v2_parser.add_argument('--v3-theta-output-high', type=float, default=1.45)
    v2_parser.add_argument('--v3-theta-state-margin-release', type=float, default=0.10)
    ...
    v2 = V3PrecisionConfig(
        ...
        theta_output_high=float(v2_args.v3_theta_output_high),
        theta_state_margin_release=float(v2_args.v3_theta_state_margin_release),
...
def make_tracker(args: argparse.Namespace, v2: V3PrecisionConfig) -> ShadowPDAV3PrecisionTracker:
    ...
    return ShadowPDAV3PrecisionTracker(cfg, osr, shadow, v2)
