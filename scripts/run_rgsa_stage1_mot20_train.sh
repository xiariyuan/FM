...
# MOT20 RGSA Stage1 training pipeline.
# Requires a MOT20 HACA v3 checkpoint produced by run_haca_v3_mot20_train.sh.
...
OUT_ROOT="${OUT_ROOT:-${REPO_ROOT}/outputs/rgsa_stage1_mot20_pipeline_${TS}}"
...
update_plan_status() {
  ...
  "${PYTHON_BIN}" "${REPO_ROOT}/scripts/upsert_experiment_plan.py" \
    ...
    --status "${status}" \
    --kind train \
    --script "scripts/run_rgsa_stage1_mot20_train.sh" \
    --dataset MOT20 \
    --split train_half_val_half \
    --tracker-family BoT-SORT \
    --variant "rgsa_stage1_mot20_train" \
    --run-root "${OUT_ROOT}" \
...
for SEQ_ID in "${DUMP_SEQS[@]}"; do
  ...
  (
    ...
    "${PYTHON_BIN}" -u tools/track.py "${DATA_ROOT}/MOT20" \
      ...
      --experiment-name "rgsa_oracle_mot20_${SEQ_NAME}_${TS}" \
      --laplace-assoc \
...
echo "[done] Stage1 checkpoint: ${STAGE1_OUT}/stage1_best.pt"
