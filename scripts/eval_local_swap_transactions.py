...
def source_to_emitted_map(source_rows,target_rows):
 ...
 for t in target_rows:buckets[row_key(t)].append(int(float(t[1])))
 out={};missing=0
 for s in source_rows:
  k=row_key(s)
  if not buckets[k]:missing+=1;continue
  out[(int(float(s[0])),int(float(s[1])))] = buckets[k].popleft()
 if missing:raise RuntimeError(f'missing non-id row matches: {missing}')
 remain=sum(len(v) for v in buckets.values())
 if remain:raise RuntimeError(f'unmatched target rows: {remain}')
 return out

def apply_local_swaps(internal_path,input_path,out_path,events,horizon):
 internal=read_rows(internal_path);rows=read_rows(input_path);imap=source_to_emitted_map(internal,rows)
 starts=defaultdict(list);ends=defaultdict(list)
 for e in events:
  starts[int(e['frame'])].append(e)
  if horizon is not None:ends[int(e['frame'])+horizon+1].append(e)
 rows_by=defaultdict(list)
 for r in rows:rows_by[int(float(r[0]))].append(r)
 mapping={};active_labels={};applied=[];skipped=[];out=[];changed=0
 def swap_labels(la,lb):
  mapping.setdefault(la,la);mapping.setdefault(lb,lb);mapping[la],mapping[lb]=mapping[lb],mapping[la]
 for fr in sorted(rows_by):
  for e in sorted(ends.get(fr,[]),key=lambda x:(int(x['track_a']),int(x['track_b']))):
   key=(int(e['frame']),int(e['track_a']),int(e['track_b']))
   labs=active_labels.pop(key,None)
   if labs:swap_labels(labs[0],labs[1])
  for e in sorted(starts.get(fr,[]),key=lambda x:(int(x['track_a']),int(x['track_b']))):
   ia=int(e['track_a']);ib=int(e['track_b']);la=imap.get((fr,ia));lb=imap.get((fr,ib))
   if la is None or lb is None or la==lb:
    skipped.append({**e,'reason':'missing_or_same_label','label_a':la,'label_b':lb});continue
   swap_labels(la,lb);active_labels[(fr,ia,ib)]=(la,lb);applied.append({**e,'label_a':la,'label_b':lb})
  emitted=[]
...
def main():
 ...
 scores=pd.read_csv(args.scores);diag=pd.read_csv(args.diagnostics);events=select_events(scores,diag);out=Path(args.out_dir);out.mkdir(parents=True,exist_ok=True);pd.DataFrame(events).to_csv(out/'selected_events.csv',index=False);summaries=[]
 for root,name in zip(args.input_roots,args.input_names):
  for horizon in [10,30,60,None]:
   tag='perm' if horizon is None else f'h{horizon}';pdir=out/f'{name}_{tag}';tdir=pdir/'track_results';tdir.mkdir(parents=True,exist_ok=True);applied=[];skipped=[];byseq=[]
   for seq in SEQS:
    ev=[e for e in events if e['seq']==seq]
    a,s,changed=apply_local_swaps(Path(args.internal_root)/f'{seq}.txt',Path(root)/f'{seq}.txt',tdir/f'{seq}.txt',ev,horizon)
   ...
   evres=evaluate(pdir,f'{name}_{tag}');summary={'name':name,'horizon':horizon,'requested':len(events),'applied':len(applied),'skipped':len(skipped),'by_seq':byseq,'eval':evres};(pdir/'summary.json').write_text(json.dumps(summary,indent=2)+'\n');summaries.append(summary)
   print(json.dumps({'name':name,'horizon':horizon,'applied':len(applied),'skipped':len(skipped),'combined':evres.get('COMBINED'),'m02':evres.get('MOT20-02'),'avg':evres.get('simple_avg_HOTA')},indent=2),flush=True)
