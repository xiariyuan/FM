...
OUT_DIR="${1:-${REPO_ROOT}/outputs/local_conflict_graph_commitmatches_hardtrigger_oracle_proxy0213_$(date +%Y%m%d_%H%M%S)}"
...
"${PYTHON_BIN}" - "${RESULT_CSV}" "${SUMMARY_CSV}" "${OUT_DIR}" "${ORACLE_JSONL}" "${TOPK}" "${MIN_DETECTIONS}" "${MIN_COMMITTED_MATCHES}" <<'PY'
import csv
import sys
...
min_committed_matches = sys.argv[7]
...
row = {
    "exp_name": "local_conflict_graph_commitmatches_hardtrigger_oracle_proxy0213",
...
"${PYTHON_BIN}" - "${PROFILE_PATH}" "${BASE_CONFIG}" "${BASE_CKPT}" "${ORACLE_JSONL}" "${TOPK}" "${MIN_DETECTIONS}" "${MIN_COMMITTED_MATCHES}" <<'PY'
import json
import sys
from pathlib import Path
...
min_committed_matches = int(sys.argv[7])
...
doc = {
    "description": "Local conflict graph matched-edge commit oracle with host fallback and hard cluster trigger on proxy0213.",
    "manifest": {
...
"${PYTHON_BIN}" - "${RESULT_CSV}" "${SUMMARY_CSV}" "${MANIFEST_PATH}" "${BASE_CKPT}" "${OUT_DIR}" "${ORACLE_JSONL}" "${TOPK}" "${MIN_DETECTIONS}" "${MIN_COMMITTED_MATCHES}" "${STATUS}" <<'PY'
import csv
import json
import sys
from pathlib import Path
...
"${PYTHON_BIN}" "${REPO_ROOT}/scripts/append_experiment_record.py" \
  ...
  --script "scripts/run_local_conflict_graph_commitmatches_hardtrigger_oracle_proxy0213.sh" \
  --dataset "MOT17" \
  --split "val0213_proxy" \
  --tracker-family "ByteTrack" \
  --variant "local_conflict_graph_commitmatches_hardtrigger_oracle_proxy0213" \
  --tag "local_conflict_graph_mainline" \
  --run-root "${OUT_DIR}" \
  --summary-csv "${SUMMARY_CSV}" \
  --checkpoint "${BASE_CKPT}" \
  --log-path "${LOG_PATH}" \
  --notes "local conflict graph matched-edge commit oracle with hard cluster trigger on proxy0213" \
  --extra "oracle_group_jsonl=${ORACLE_JSONL}" "graph_mode=oracle_commit_matches" "graph_topk=${TOPK}" "graph_min_detections=${MIN_DETECTIONS}" "graph_min_committed_matches=${MIN_COMMITTED_MATCHES}"

if ! "${PYTHON_BIN}" "${REPO_ROOT}/scripts/post_experiment_pro_bundle.py" \
  --run-root "${OUT_DIR}" \
  --tag "local_conflict_graph_commitmatches_hardtrigger_oracle_bundle" \
  --label "local_conflict_graph_commitmatches_hardtrigger_oracle_proxy0213" \
  --status "$( [[ "${STATUS}" == "success" ]] && echo ok || echo failed )"; then
  echo "[local-conflict-graph-commitmatches-hardtrigger-oracle] warning: failed to build Pro review bundle for ${OUT_DIR}" >&2
fi

echo "[local-conflict-graph-commitmatches-hardtrigger-oracle] status=${STATUS} out_dir=${OUT_DIR}"
